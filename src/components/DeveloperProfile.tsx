    ...
    >
      ...
      >
        <div className="flex items-center gap-4">
          <div
            className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-2xl shadow-md shrink-0 ${
              isCustom
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span
                className={`px-2 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-1 border ${
                  isCustom
              ...
              >
                ...
                {language === "bn"
                  ? "ভেরিফায়েড সফটওয়্যার ভেন্ডর"
                  : "Verified Software Vendor"}
              </span>
            </div>
            <p
              className={`text-xs mt-0.5 ${isCustom ? "text-purple-300/80" : isOcean ? "text-sky-300/80" : isDark ? "text-slate-400" : "text-slate-500"}`}
            >
              {language === "bn"
      ...
      <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        >
          <div>
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider mb-3">
              <Phone
                className={`w-4 h-4 ${isCustom ? "text-amber-400" : isOcean ? "text-sky-400" : "text-emerald-500"}`}
              />
              <span>
                {language === "bn"
                  ? "কারিগরি সহায়তা ও হটলাইন"
            ...
            <div className="space-y-2 text-xs">
              <div className="flex items-center justify-between">
                <span className="opacity-70">
                  {language === "bn" ? "জরুরি হটলাইন:" : "Helpline:"}
                </span>
                <a
              ...
              <div className="flex items-center justify-between">
                <span className="opacity-70">
                  {language === "bn" ? "ইমেইল:" : "Support Email:"}
                </span>
                <span className="font-mono text-xs opacity-90">
                  support@pjcautomation.com
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="opacity-70">
                  {language === "bn" ? "সহায়তা সময়:" : "Availability:"}
                </span>
                <span className="font-medium">
                  {language === "bn"
                    ? "২৪/৭ জরুরি সরকারি সেবা"
                    : "24/7 GovTech Priority SLA"}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-2 text-xs">
              <div className="flex items-start justify-between gap-2">
                ...
                <span className="font-semibold text-right">
                  Rangamati Hill Tracts, Chattogram, Bangladesh
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="opacity-70">
                  {language === "bn" ? "সেবা অঞ্চল:" : "Coverage:"}
                </span>
                <span className="font-medium">
                  {language === "bn"
                    ? "সমগ্র বাংলাদেশ (সকল সরকারি দপ্তর)"
                    : "All Government Administrative Offices"}
                </span>
              </div>
            </div>
          </div>
        ...
        <div
          className={`p-4 rounded-xl border flex flex-col justify-between md:col-span-2 lg:col-span-1 ${
            isCustom
              ? "bg-[#1f173d] border-[#382b61]"
              : isOcean
                ? "bg-sky-900/30 border-sky-800/60"
        ...
        >
          <div>
            ...
            <div className="space-y-2 text-xs">
              <div className="flex items-center justify-between">
                <span className="opacity-70">
                  {language === "bn" ? "ডাটাবেজ ও ক্লাউড:" : "Database Engine:"}
                </span>
                <span className="font-mono text-xs font-semibold">
                  Google Apps Script & Cloud SQL
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="opacity-70">
                  {language === "bn" ? "নিরাপত্তা আর্কিটেকচার:" : "Security:"}
                </span>
                <span className="font-medium">
                  {language === "bn"
              ...
              <div className="flex items-center justify-between">
                <span className="opacity-70">
                  {language === "bn" ? "অডিট ট্রেইল:" : "Audit Log:"}
                </span>
                <span className="font-medium">
                  {language === "bn"
                    ? "সম্পূর্ণ ইমিউটেবল ট্র্যাকিং"
      ...
      <div
        className={`px-6 py-3 border-t flex flex-wrap items-center justify-between gap-3 text-xs opacity-80 ${
          isCustom
            ? "border-[#2b1f4d] bg-[#140f29]"
            : isOcean
              ? "border-sky-900 bg-sky-950/90"
              : isDark
