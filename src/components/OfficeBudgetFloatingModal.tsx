...
import { Allocation, Expense, Office, Category, FinancialYear, User, SystemSettings } from "../types";
import { 
  X, Printer, Download, Calendar, ChevronDown, ChevronRight, 
  Building2, Wallet, TrendingDown, Layers, Clock, AlertCircle, 
  CheckCircle2, RotateCcw, Edit2, Trash2, Maximize2, Minimize2 
} from "lucide-react";
...
export function OfficeBudgetFloatingModal({
  ...
  const categoryBudgetData = useMemo(() => {
    ...
    return Object.values(map)
      .filter(item => {
        const hasData = item.currentAllocation !== 0 || item.totalExpense !== 0 || item.rows.length > 0;
        if (searchCategory.trim()) {
          const query = searchCategory.toLowerCase();
          const matchName = (item.category?.name || "").toLowerCase().includes(query);
          const matchHead = (item.category?.budgetHead || "").toLowerCase().includes(query);
          return (matchName || matchHead);
        }
        return hasData;
      })
      .sort((a, b) => (a.category?.budgetHead || "").localeCompare(b.category?.budgetHead || ""));
  ...
  const areAllExpanded = categoryBudgetData.length > 0 && categoryBudgetData.every(item => !!expandedCategories[item.category.id]);
  ...
  const handleExportCSV = () => {
    ...
    csv += "SL,Budget Head,Category Name,Initial Allocation,Additional Allocation,Adjustment,Current Total Allocation,Total Expense,Balance\n";
    ...
    link.setAttribute("download", `Budget_Statement_${selectedOfficeObj?.code || "All"}_${new Date().toISOString().split("T")[0]}.csv`);
    ...
    <div className="fixed inset-0 bg-slate-950/75 backdrop-blur-md z-50 flex items-center justify-center p-2 sm:p-4 md:p-6 overflow-y-auto animate-in fade-in duration-200">
      ...
      >
        ...
        <div className={`p-4 sm:p-5 border-b flex flex-wrap items-center justify-between gap-3 shrink-0 ${
          isCustom ? "bg-[#1a1336] border-[#312554]" : isDark ? "bg-slate-850 border-slate-800" : "bg-slate-50/90 border-slate-200"
        }`}>
          ...
          <div className="flex items-center gap-3 min-w-0">
            <div className={`p-2.5 rounded-2xl shrink-0 shadow-sm ${
              ...
              {isAllOffices ? <Layers className="w-5 h-5" /> : <Building2 className="w-5 h-5" />}
            </div>

            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="text-base sm:text-lg font-bold truncate">
                  {isAllOffices ? (language === "bn" ? "সকল কার্যালয় ও শাখার সমন্বিত বাজেট তালিকা" : "All Offices & Branches Consolidated Budget") : selectedOfficeObj?.name}
                </h3>
                
                {!isAllOffices && selectedOfficeObj && (
                  <span className={`text-xs px-2.5 py-0.5 rounded-full font-mono font-bold ${
                    isCustom ? "bg-[#2d2254] text-amber-300 border border-[#48377e]" : isDark ? "bg-slate-800 text-emerald-400 border border-slate-700" : "bg-emerald-50 text-emerald-800 border border-emerald-200"
                  }`}>
                ...
                <span className={`text-[11px] px-2 py-0.5 rounded-full font-semibold ${
                  isCustom ? "bg-purple-900/40 text-purple-200 border border-purple-800/50" : isDark ? "bg-slate-800 text-slate-300 border border-slate-700" : "bg-slate-200/80 text-slate-700"
                }`}>
                  {t.financialYear}: {currentFYObj?.name}
                </span>
              </div>

              <p className={`text-xs mt-0.5 flex items-center gap-2 ${isCustom ? "text-purple-300/80" : isDark ? "text-slate-400" : "text-slate-500"}`}>
                <span>{language === "bn" ? "খাতভিত্তিক বর্তমান বরাদ্দ, অতিরিক্ত/সমন্বয় ও ব্যয়ের লাইভ হিসাব" : "Category-wise current budget, tranches, & expenditure overview"}</span>
                <span>•</span>
                <span className="font-semibold text-emerald-500">{categoryBudgetData.length} {language === "bn" ? "টি খাত" : "Categories"}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            >
              {areAllExpanded ? <ChevronDown className="w-3.5 h-3.5" /> : <ChevronRight className="w-3.5 h-3.5" />}
              <span className="hidden sm:inline">
                {areAllExpanded 
                  ? (language === "bn" ? "সব কলাপস করুন" : "Collapse All") 
                  : (language === "bn" ? "সব কলাপস খুলুন" : "Expand All")}
              </span>
            </button>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-5 flex-1 custom-scrollbar">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3">
            {/* Total Budget Card */}
            <div className={`p-4 rounded-2xl border flex flex-col justify-between gap-2 shadow-sm ${
              isCustom 
              ...
              <div className="flex items-center justify-between">
                <span className={`text-xs font-semibold ${isCustom ? "text-purple-300" : isDark ? "text-slate-400" : "text-emerald-800"}`}>
                  {language === "bn" ? "মোট বর্তমান বরাদ্দ" : "Current Total Allocation"}
                </span>
                <Wallet className="w-4 h-4 text-emerald-500" />
              </div>
              <div className="space-y-0.5">
                <div className={`text-lg sm:text-xl font-black font-mono tracking-tight ${isCustom ? "text-amber-300" : isDark ? "text-white" : "text-emerald-950"}`}>
                  {formatCurrency(overallMetrics.currentAllocation)}
                </div>
                <div className={`text-[10px] font-medium flex items-center gap-1.5 ${isCustom ? "text-purple-400" : isDark ? "text-slate-400" : "text-slate-500"}`}>
                  <span>মূল: {formatNumber(overallMetrics.initial)}</span>
                  <span>•</span>
                  <span>অতিরিক্ত: +{formatNumber(overallMetrics.additional)}</span>
                </div>
              </div>
            </div>
            ...
            <div className={`p-4 rounded-2xl border flex flex-col justify-between gap-2 shadow-sm ${
              ...
              <div className="flex items-center justify-between">
                <span className={`text-xs font-semibold ${isCustom ? "text-purple-300" : isDark ? "text-slate-400" : "text-rose-800"}`}>
              ...
              <div className="space-y-0.5">
                ...
                <div className={`text-[10px] font-medium flex items-center gap-1.5 ${isCustom ? "text-purple-400" : isDark ? "text-slate-400" : "text-slate-500"}`}>
                  <span>অনুমোদিত: {formatNumber(overallMetrics.approvedExpense)}</span>
                  {overallMetrics.pendingExpense > 0 && (
                    <>
                      <span>•</span>
                      <span className="text-amber-500 font-bold">পেন্ডিং: {formatNumber(overallMetrics.pendingExpense)}</span>
                    </>
                  )}
                </div>
              </div>
            ...
            <div className={`p-4 rounded-2xl border flex flex-col justify-between gap-2 shadow-sm ${
              ...
              <div className="flex items-center justify-between">
                <span className={`text-xs font-semibold ${isCustom ? "text-purple-300" : isDark ? "text-slate-400" : "text-blue-800"}`}>
              ...
              <div className="space-y-0.5">
                <div className={`text-lg sm:text-xl font-black font-mono tracking-tight ${
                  overallMetrics.balance < 0 ? "text-rose-500" : "text-emerald-500 dark:text-emerald-400"
                }`}>
                  {formatCurrency(overallMetrics.balance)}
                </div>
                <div className={`text-[10px] font-medium ${isCustom ? "text-purple-400" : isDark ? "text-slate-400" : "text-slate-500"}`}>
                  {overallMetrics.balance < 0 ? (
                    <span className="text-rose-500 font-bold">⚠️ বাজেট ঘাটতি</span>
                  ) : (
                    <span className="text-emerald-600 dark:text-emerald-400 font-medium">✓ বাজেট পর্যাপ্ত</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className={`p-4 rounded-2xl border flex flex-col justify-between gap-2 shadow-sm ${
              ...
              <div className="flex items-center justify-between">
                <span className={`text-xs font-semibold ${isCustom ? "text-purple-300" : isDark ? "text-slate-400" : "text-slate-600"}`}>
                  {language === "bn" ? "খাত ও বরাদ্দের সংখ্যা" : "Categories & Tranches"}
                </span>
                <Layers className="w-4 h-4 text-purple-400" />
              </div>
              <div className="space-y-0.5">
                <div className={`text-lg sm:text-xl font-black font-mono tracking-tight ${isCustom ? "text-purple-100" : isDark ? "text-slate-100" : "text-slate-900"}`}>
                  {formatNumber(overallMetrics.categoryCount)} <span className="text-xs font-normal opacity-70">খাত</span>
                </div>
                <div className={`text-[10px] font-medium ${isCustom ? "text-purple-400" : isDark ? "text-slate-400" : "text-slate-500"}`}>
                  মোট বরাদ্দ ভাউচার: {formatNumber(overallMetrics.allocationCount)} টি
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto">
                <div className="flex items-center gap-1.5 text-xs font-bold text-emerald-500 shrink-0">
                  ...
                  <span>{language === "bn" ? "তারিখ ফিল্টার:" : "Date Filter:"}</span>
                </div>

                <div className="flex items-center gap-1.5 flex-1 sm:flex-initial">
                  <span className={`text-[11px] font-medium ${isCustom ? "text-purple-300" : isDark ? "text-slate-400" : "text-slate-500"}`}>
                ...
                <div className="flex items-center gap-1 flex-wrap">
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-lg text-[11px] font-semibold border transition ${
                      isCustom ? "bg-[#251b47] border-[#443370] text-purple-200 hover:text-amber-300" : isDark ? "bg-slate-800 border-slate-700 text-slate-300 hover:text-emerald-400" : "bg-white border-slate-200 text-slate-700 hover:bg-slate-100"
                    }`}
                  >
              ...
              <div className="w-full lg:w-64">
                <input
                  type="text"
                  placeholder={language === "bn" ? "খাতের নাম বা কোড দিয়ে খুঁজুন..." : "Search category or code..."}
          ...
          <div className={`rounded-2xl border overflow-hidden shadow-sm transition-colors ${
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                  ...
                  <tr className={`border-b text-xs uppercase tracking-wider font-bold ${
                    ...
                    <th className="p-3.5 font-bold min-w-[200px]">
                      <div className="flex items-center gap-1.5">
                        <span>{language === "bn" ? "বাজেট খাত ও বিবরণ" : "Budget Category & Head"}</span>
                        <span className={`text-[10px] font-normal normal-case px-1.5 py-0.5 rounded ${
                          isCustom ? "bg-[#2d2254] text-purple-300" : isDark ? "bg-slate-700 text-slate-300" : "bg-slate-200 text-slate-600"
                        }`}>
                          {language === "bn" ? "ক্লিক করে বিস্তারিত দেখুন" : "Click to toggle"}
                        </span>
                      </div>
                    </th>
                    <th className="p-3.5 font-bold text-right text-emerald-600 dark:text-emerald-400">
                      {language === "bn" ? "বর্তমান বরাদ্দ" : "Current Total Budget"}
                    </th>
                    ...
                    <tr>
                      <td colSpan={6} className={`p-10 text-center ${
                        ...
                        <p className="font-semibold text-sm">
                          {language === "bn" ? "কোনো বাজেট বরাদ্দ বা খাতের তথ্য পাওয়া যায়নি।" : "No budget allocation or category records found."}
                        </p>
                        <p className="text-xs mt-1 opacity-70">
                          {language === "bn" ? "তারিখ ফিল্টার রিসেট করুন বা নতুন বরাদ্দ প্রদান করুন।" : "Please reset the date filter or add an allocation."}
                        </p>
                      </td>
                    </tr>
                  ) : (
                        ...
                        <React.Fragment key={item.category.id}>
                          ...
                          >
                            ...
                            <td className="p-3.5 text-center font-mono">
                              <button
                                ...
                                className={`p-1 rounded-lg transition-transform duration-200 inline-flex items-center justify-center ${
                                  isExpanded ? "rotate-90 text-emerald-500" : "text-slate-400 group-hover:text-emerald-500"
                                }`}
                              >
                            ...
                            <td className="p-3.5">
                              <div className="font-bold text-xs sm:text-sm flex items-center gap-2">
                                <span className={isCustom ? "text-purple-100" : isDark ? "text-white" : "text-slate-900"}>
                                  {item.category?.name || "N/A"}
                                </span>
                                {item.rows.length > 1 && (
                                  <span className={`text-[10px] px-1.5 py-0.2 rounded font-mono font-medium ${
                                    isCustom ? "bg-[#33245e] text-amber-300" : isDark ? "bg-slate-700 text-slate-300" : "bg-slate-100 text-slate-600"
                                  }`}>
                                    {item.rows.length} {language === "bn" ? "টি এন্ট্রি" : "tranches"}
                                  </span>
                                )}
                              </div>
                            ...
                            {/* Available Balance */}
                            <td className={`p-3.5 text-right font-mono font-bold text-xs sm:text-sm ${
                              item.balance < 0 ? "text-rose-500 font-extrabold" : "text-emerald-500 dark:text-emerald-400"
                            }`}>
                            ...
                            <td className="p-3.5 text-center">
                              {item.balance < 0 ? (
                                ...
                                <span className="inline-block px-2 py-0.5 rounded-full text-[10px] font-medium bg-slate-500/15 text-slate-400">
                                  {language === "bn" ? "বরাদ্দ নেই" : "No Alloc"}
                                </span>
                              ) : item.balance === 0 ? (
                                <span className="inline-block px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-500/15 border border-amber-500/30 text-amber-400">
                                  {language === "bn" ? "সম্পূর্ণ ব্যয়" : "Exhausted"}
                                </span>
                              ) : (
                          ...
                          {isExpanded && (
                            <tr className={`border-b ${
                              ...
                              <td colSpan={6} className="p-3 sm:p-4 pl-6 sm:pl-12 border-l-4 border-emerald-500">
                                <div className="space-y-3">
                                  ...
                                  <div className="flex flex-wrap items-center justify-between gap-2">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className={`font-bold text-xs ${isCustom ? "text-purple-200" : isDark ? "text-slate-200" : "text-slate-700"}`}>
                                        {language === "bn" ? "বরাদ্দ বিভাজন (মূল, অতিরিক্ত ও সমন্বয় বিবরণী)" : "Allocation Tranche Breakdown"}
                                      </span>
                                    </div>

                                    {/* Breakdown summary pills */}
                                    <div className="flex items-center gap-2 text-[11px] font-mono">
                                      <span className={`px-2 py-0.5 rounded ${isCustom ? "bg-[#1d163a] text-purple-300" : isDark ? "bg-slate-800 text-slate-300" : "bg-white border text-slate-700"}`}>
                                        মূল: <strong>{formatCurrency(item.initial)}</strong>
                                      </span>
                                      <span className={`px-2 py-0.5 rounded ${isCustom ? "bg-[#1d163a] text-blue-300" : isDark ? "bg-slate-800 text-blue-300" : "bg-white border text-blue-700"}`}>
                                        অতিরিক্ত: <strong>+{formatCurrency(item.additional)}</strong>
                                      </span>
                                      <span className={`px-2 py-0.5 rounded ${
                                        item.adjustment < 0 
                                          ...
                                          : item.adjustment > 0 
                                          ? "text-amber-400" 
                                          : isCustom ? "text-purple-400" : "text-slate-500"
                                      } ${isCustom ? "bg-[#1d163a]" : isDark ? "bg-slate-800" : "bg-white border"}`}>
                                        সমন্বয়: <strong>{item.adjustment > 0 ? "+" : ""}{formatCurrency(item.adjustment)}</strong>
                                      </span>
                                    </div>
                                  </div>
                                  ...
                                  {hasTranches ? (
                                    <div className={`rounded-xl border overflow-hidden ${
                                      isCustom ? "bg-[#171030] border-[#312554]" : isDark ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"
                                    }`}>
                                      <table className="w-full text-left text-xs border-collapse">
                                        <thead>
                                          <tr className={`border-b text-[10px] uppercase tracking-wider font-semibold ${
                                            ...
                                            <th className="p-2.5 pl-3">ধরন (Type)</th>
                                            <th className="p-2.5">তারিখ (Date)</th>
                                            <th className="p-2.5">স্মারক নম্বর (Ref No)</th>
                                            <th className="p-2.5">বিবরণ / মন্তব্য (Remarks)</th>
                                            <th className="p-2.5 text-right pr-3">টাকার পরিমাণ (Amount)</th>
                                            {isHeadOffice && <th className="p-2.5 text-center w-16">অ্যাকশন</th>}
                                          </tr>
                                        </thead>
                                        <tbody className={`divide-y font-mono ${
                                          ...
                                          {item.rows.map((row) => (
                                            <tr key={row.id} className={`transition-colors ${
                                              ...
                                              {/* Date */}
                                              <td className="p-2.5 text-slate-400">{row.date || "N/A"}</td>

                                              {/* Ref No */}
                                              <td className="p-2.5 font-bold font-mono">
                                                <span className={isCustom ? "text-purple-200" : isDark ? "text-slate-200" : "text-slate-800"}>
                                                  {row.referenceNo || "—"}
                                                </span>
                                              </td>
                                              ...
                                              {/* Amount */}
                                              <td className={`p-2.5 pr-3 text-right font-bold text-xs ${
                                                ...
                                                {formatCurrency(Number(row.allocatedAmount || 0))}
                                              </td>
                                                ...
                                                <td className="p-2.5 text-center">
                                                  <div className="flex items-center justify-center gap-1.5">
                                                    ...
                                                    {onDeleteAllocation && (
                                                      <button
                                                        type="button"
                                                        onClick={() => onDeleteAllocation(row.id)}
                                    ...
                                    <div className="p-3 text-center text-xs text-slate-400 italic">
                                      এই খাতে নির্বাচিত সময়কালে কোনো বরাদ্দ এন্ট্রি পাওয়া যায়নি।
                                    </div>
                                  )}
                                </div>
                              </td>
                ...
                {categoryBudgetData.length > 0 && (
                  <tfoot>
                    <tr className={`border-t font-bold text-xs sm:text-sm ${
                      isCustom 
                      ...
                      <td colSpan={2} className="p-3.5 font-bold">
                        <div className="flex items-center justify-between">
                          <span>{language === "bn" ? "সর্বমোট বাজেট ও ব্যয় সংক্ষেপ:" : "GRAND TOTAL:"}</span>
                          <span className="text-xs font-normal opacity-70">({categoryBudgetData.length}টি খাত)</span>
                        </div>
                      </td>
        ...
        <div className={`p-3 sm:p-4 border-t flex flex-wrap items-center justify-between gap-3 text-xs shrink-0 ${
          isCustom ? "bg-[#171030] border-[#312554] text-purple-300" : isDark ? "bg-slate-850 border-slate-800 text-slate-400" : "bg-slate-50 border-slate-200 text-slate-600"
        }`}>
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>{language === "bn" ? "প্রিন্ট / PDF সংরক্ষণ করুন" : "Print / Save PDF"}</span>
            </button>
            <button
              type="button"
              onClick={onClose}
      ...
      {/* ========================================================================= */}
      <div id="printable-budget-statement" ref={printRef} className="hidden print:block fixed inset-0 bg-white text-black p-8 font-sans z-[9999]">
        ...
        <div className="text-center mb-6 border-b-2 border-black pb-4">
          ...
          <h2 className="text-base font-bold text-gray-800 mt-1">
            {isAllOffices ? "সকল শাখা ও কার্যালয়ের সমন্বিত বাজেট বরাদ্দ ও ব্যয় বিবরণী" : `${selectedOfficeObj?.name} (${selectedOfficeObj?.code})`}
          </h2>
          <div className="flex justify-between items-center text-xs font-semibold text-gray-700 mt-2 px-2">
            ...
            <span>
              বিবরণী সময়কাল: {filterDateFrom || currentFYObj?.startDate || "শুরু"} হতে {filterDateTo || currentFYObj?.endDate || "বর্তমান"}
            </span>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6 text-center border border-black p-3 bg-gray-50">
          <div>
            <div className="text-xs font-bold text-gray-600">মোট বর্তমান বরাদ্দ</div>
            <div className="text-base font-black text-black">{formatCurrency(overallMetrics.currentAllocation)}</div>
          </div>
          <div>
            <div className="text-xs font-bold text-gray-600">মোট ব্যয়িত অর্থ</div>
            <div className="text-base font-black text-black">{formatCurrency(overallMetrics.totalExpense)}</div>
          </div>
          <div>
            <div className="text-xs font-bold text-gray-600">অবশিষ্ট স্থিতি</div>
            <div className="text-base font-black text-black">{formatCurrency(overallMetrics.balance)}</div>
          </div>
        </div>
        ...
        <table className="w-full text-left text-xs border-collapse border border-black mb-8">
          <thead>
            <tr className="bg-gray-200 border-b border-black text-black font-bold">
              <th className="p-2 border-r border-black text-center w-8">ক্রম</th>
              <th className="p-2 border-r border-black w-24">কোড</th>
              <th className="p-2 border-r border-black">বাজেট খাত ও বিবরণ</th>
              <th className="p-2 border-r border-black text-right">মূল বরাদ্দ</th>
              ...
              <th className="p-2 border-r border-black text-right font-black">বর্তমান বরাদ্দ</th>
          ...
          <tbody className="divide-y divide-black font-mono">
            {categoryBudgetData.map((item, idx) => (
              <tr key={item.category?.id || idx} className="border-b border-black">
                <td className="p-1.5 border-r border-black text-center font-sans">{idx + 1}</td>
                <td className="p-1.5 border-r border-black font-bold">{item.category?.budgetHead || ""}</td>
                <td className="p-1.5 border-r border-black font-sans font-medium">{item.category?.name || ""}</td>
                <td className="p-1.5 border-r border-black text-right">{formatCurrency(item.initial)}</td>
                <td className="p-1.5 border-r border-black text-right">+{formatCurrency(item.additional)}</td>
                <td className="p-1.5 border-r border-black text-right">{item.adjustment > 0 ? "+" : ""}{formatCurrency(item.adjustment)}</td>
                <td className="p-1.5 border-r border-black text-right font-bold bg-gray-100">{formatCurrency(item.currentAllocation)}</td>
                <td className="p-1.5 border-r border-black text-right font-semibold">{formatCurrency(item.totalExpense)}</td>
                <td className="p-1.5 text-right font-black">{formatCurrency(item.balance)}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr className="bg-gray-200 font-bold border-t-2 border-black text-black">
              <td colSpan={3} className="p-2 border-r border-black font-sans font-extrabold text-right">
                সর্বমোট (GRAND TOTAL):
              </td>
              <td className="p-2 border-r border-black text-right font-mono font-bold">{formatCurrency(overallMetrics.initial)}</td>
              <td className="p-2 border-r border-black text-right font-mono font-bold">+{formatCurrency(overallMetrics.additional)}</td>
              <td className="p-2 border-r border-black text-right font-mono font-bold">{overallMetrics.adjustment > 0 ? "+" : ""}{formatCurrency(overallMetrics.adjustment)}</td>
              <td className="p-2 border-r border-black text-right font-mono font-black bg-gray-300">{formatCurrency(overallMetrics.currentAllocation)}</td>
              <td className="p-2 border-r border-black text-right font-mono font-bold">{formatCurrency(overallMetrics.totalExpense)}</td>
              <td className="p-2 text-right font-mono font-black">{formatCurrency(overallMetrics.balance)}</td>
            </tr>
          </tfoot>
        </table>
        ...
        <div className="grid grid-cols-2 gap-12 mt-16 pt-8 text-xs font-sans">
          <div className="text-center">
            ...
            <div className="text-[10px] text-gray-600 mt-0.5">স্বাক্ষর ও তারিখ</div>
          </div>
          <div className="text-center">
            <div className="border-t border-black pt-2 w-48 mx-auto font-bold">
