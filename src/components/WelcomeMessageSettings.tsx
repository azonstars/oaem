...
const DEFAULT_WELCOME_MESSAGES: Record<
...
export function WelcomeMessageSettings({
  ...
  const [previewSlot, setPreviewSlot] = useState<
    "morning" | "afternoon" | "evening" | "night"
  >("morning");
  const [previewOfficeId, setPreviewOfficeId] = useState<string>(
    currentUser.officeId || offices[0]?.id || "",
  );
  ...
  const currentOffice =
    offices.find((o) => o.id === (previewOfficeId || currentUser.officeId)) ||
    offices[0];
  const activeFY = financialYears.find((f) => f.isActive) || financialYears[0];

  const currentNotices =
    settingsForm.notices && settingsForm.notices.length > 0
  ...
  const getSlotMessage = (
    ...
    return {
      line1:
        custom?.line1 !== undefined
          ? custom.line1
          : DEFAULT_WELCOME_MESSAGES[slotKey].line1,
      line2:
        custom?.line2 !== undefined
          ? custom.line2
          : DEFAULT_WELCOME_MESSAGES[slotKey].line2,
    };
  };
  ...
  const handleFieldChange = (
    ...
    setSettingsForm((prev) => {
      ...
      const currentSlot = currentMessages[slotKey] || {
        ...DEFAULT_WELCOME_MESSAGES[slotKey],
      };
      return {
        ...prev,
        welcomeMessages: {
  ...
  const renderPreview = (
    ...
    return template
      ...
      .replace(
        /{greeting}/g,
        slot ? greetingMap[slot] : language === "bn" ? "শুভেচ্ছা" : "Greetings",
      )
      .replace(
        /{institution}/g,
        settingsForm.institutionName ||
  ...
  const slotsConfig: Array<{
    ...
    },
    {
      ...
      icon: SunMedium,
      colorClasses:
        "from-amber-500/20 to-yellow-500/20 text-yellow-600 dark:text-yellow-400 border-yellow-500/30",
    },
    {
      id: "evening",
    ...
    },
    {
      ...
      icon: Moon,
      colorClasses:
        "from-blue-500/20 to-slate-500/20 text-blue-600 dark:text-blue-400 border-blue-500/30",
    },
  ];
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">
                {language === "bn"
                  ? "ওয়েলকাম মেসেজ ও নোটিশ/বিজ্ঞপ্তি কনফিগারেশন"
                  : "Welcome Message & Notice Configuration"}
              </h2>
      ...
      >
        <div className="flex items-center justify-between gap-2 mb-2">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs font-bold text-slate-900 dark:text-white">
              {language === "bn"
                ? "উপলব্ধ ডায়নামিক প্লেসহোল্ডার (Click to Insert)"
                : "Available Dynamic Placeholders"}
            </span>
          </div>
          <span className="text-xs opacity-60">
            {language === "bn"
              ? "ক্লিক করলেই সক্রিয় ইনপুটে যুক্ত হবে"
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          {[
            {
              tag: "{name}",
              labelBn: "ব্যবহারকারীর নাম",
              labelEn: "Officer's Name",
            },
            {
              tag: "{system}",
              labelBn: "সিস্টেম নাম",
              labelEn: "System Title",
            },
            {
              tag: "{office}",
              labelBn: "অফিস পরিচিতি",
              labelEn: "Assigned Office",
            },
            {
              tag: "{fiscal_year}",
              labelBn: "চলতি অর্থবছর",
              labelEn: "Fiscal Year",
            },
            {
              tag: "{greeting}",
              labelBn: "সময়ভিত্তিক অভিবাদন",
              labelEn: "Greeting Phrase",
            },
            {
              tag: "{institution}",
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-base font-bold text-slate-900 dark:text-white">
              {language === "bn"
                ? "১. ড্যাশবোর্ড ওয়েলকাম মেসেজ সেটিংস (৪টি সময়ভিত্তিক স্লট)"
                : "1. Dashboard Welcome Message Slots"}
            </h3>
              ...
              >
                <div>
                  ...
                  <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-200/50 dark:border-slate-800">
                    <div className="flex items-center gap-2.5">
                      <div
                        className={`p-2 rounded-xl border bg-gradient-to-br ${slot.colorClasses}`}
                      >
                  ...
                  <div className="space-y-1.5 mb-3.5">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleFieldChange(slot.id, "line1", e.target.value)
                      }
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleFieldChange(slot.id, "line2", e.target.value)
                      }
      ...
      <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-800">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">
                {language === "bn"
                  ? "২. ঘূর্ণায়মান নোটিশ ও বিজ্ঞপ্তি ব্যবস্থাপনা (Header Notice Bar)"
                  : "2. Rotating Notice & Announcements Management"}
              </h3>
              <p className="text-xs opacity-70">
                {language === "bn"
          ...
          >
            <RotateCcw className="w-3.5 h-3.5" />
            <span>
              {language === "bn" ? "বিজ্ঞপ্তি ডিফল্ট রিসেট" : "Reset Notices"}
            </span>
          </button>
        </div>
        ...
        >
          <input
            ...
            placeholder={
              language === "bn"
                ? "নতুন নোটিশ বা বিজ্ঞপ্তি লিখুন (প্লেসহোল্ডার সমর্থিত)..."
                : "Write a new announcement (supports placeholders)..."
            }
          ...
          >
            ...
            <span>
              {language === "bn" ? "বিজ্ঞপ্তি যুক্ত করুন" : "Add Notice"}
            </span>
          </button>
        </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  <input
                    ...
                    value={notice}
                    onFocus={() =>
                      setFocusedField({ type: "notice", index: idx })
                    }
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 pb-3 border-b border-slate-200/60 dark:border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-slate-900 dark:text-white">
              {language === "bn"
                ? "লাইভ সিমুলেশন ও পূর্বরূপ (Live Preview Sandbox)"
              ...
              {(["morning", "afternoon", "evening", "night"] as const).map(
                (s) => (
                  ...
                  >
                    ...
                    {s === "afternoon" &&
                      (language === "bn" ? "☀️ অপরাহ্ন" : "Afternoon")}
                    {s === "evening" &&
        ...
        <div className="space-y-3">
          <div className="text-xs font-bold uppercase tracking-wider opacity-60 flex items-center gap-1.5">
            ...
            <span>
              {language === "bn"
                ? "ড্যাশবোর্ড হিরো ব্যানার (Dashboard Hero Banner):"
                : "Dashboard Hero Banner Preview:"}
            </span>
          </div>

          <div
            className={`p-5 rounded-2xl border shadow-inner transition-colors flex flex-col md:flex-row justify-between items-start md:items-center gap-4 ${
              isCustom
          ...
          >
            <div className="min-w-0">
              ...
              <p className="text-xs font-mono mt-1 opacity-70 text-amber-300 flex items-center gap-1">
                ...
                হিসাব সূত্র: প্রারম্ভিক স্থিতি + বরাদ্দ + অতিরিক্ত বরাদ্দ ±
                সমন্বয় - ব্যয় = অবশিষ্ট স্থিতি
              </p>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex overflow-y-auto p-4 sm:p-6">
          <div className="m-auto bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl border border-slate-200">
            <h3 className="text-base font-bold text-slate-900 mb-2">
              {language === "bn" ? "নিশ্চিত করুন" : "Confirm Reset"}
            </h3>
            <p className="text-sm text-slate-600 mb-6">
              {language === "bn"
                ? "আপনি কি সমস্ত ওয়েলকাম মেসেজ পূর্বনির্ধারিত ডিফল্ট মানে রিসেট করতে চান?"
                : "Reset all welcome messages to defaults?"}
            </p>
            <div className="flex justify-end gap-2">
              <button
                onClick={() => setResetWelcomeConfirm(false)}
                className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-xl font-medium text-sm"
              >
                {t.cancel}
              </button>
              <button
                onClick={confirmResetWelcome}
                className="px-5 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-semibold shadow text-sm"
              >
      ...
      {resetNoticesConfirm && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex overflow-y-auto p-4 sm:p-6">
          <div className="m-auto bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl border border-slate-200">
            ...
            <p className="text-sm text-slate-600 mb-6">
              {language === "bn"
                ? "আপনি কি সমস্ত বিজ্ঞপ্তি পূর্বনির্ধারিত ডিফল্ট মানে রিসেট করতে চান?"
                : "Reset all notices to defaults?"}
            </p>
            <div className="flex justify-end gap-2">
              <button
                onClick={() => setResetNoticesConfirm(false)}
                className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-xl font-medium text-sm"
              >
                {t.cancel}
              </button>
              <button
                onClick={confirmResetNotices}
                className="px-5 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-semibold shadow text-sm"
              >
                {language === "bn" ? "হ্যাঁ, রিসেট করুন" : "Yes, Reset"}
              </button>
