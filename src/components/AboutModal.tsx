...
import { X, Code, Phone, MapPin, ShieldCheck, FileSpreadsheet, Server, Layers } from "lucide-react";
...
export function AboutModal({ isOpen, onClose, systemSettings }: AboutModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex overflow-y-auto p-4 sm:p-6">
      ...
      >
        ...
        {/* Modal Header */}
        <div className={`flex items-center gap-3.5 pb-4 border-b mb-5 ${
          isCustom ? "border-[#322652]" : isDark ? "border-slate-800" : "border-slate-200"
        }`}>
          ...
          <div className="min-w-0 flex-1">
            <h3 className="font-bold text-base truncate">
              {systemSettings?.webAppName || (language === "bn" ? "অফিস বরাদ্দ ও ব্যয় ব্যবস্থাপনা সিস্টেম" : "Office Allocation & Expense System")}
            </h3>
            <p className={`text-xs font-medium ${isCustom ? "text-amber-300" : isDark ? "text-emerald-400" : "text-emerald-700"}`}>
        ...
        <div className="space-y-4 text-xs leading-relaxed">
          ...
          >
            <div className="font-semibold mb-1 flex items-center gap-1.5 text-sm">
              <ShieldCheck className={`w-4 h-4 ${isCustom ? "text-amber-400" : "text-emerald-500"}`} />
              <span className={isCustom ? "text-purple-100" : isDark ? "text-slate-100" : "text-slate-900"}>
          ...
          >
            <div className="flex items-center gap-2.5 mb-3">
              <div className={`p-1.5 rounded-lg text-white shadow-sm ${
                isCustom ? "bg-purple-700" : isDark ? "bg-emerald-700" : "bg-emerald-600"
              }`}>
                <Code className="w-4 h-4" />
              </div>
              <div>
                <h4 className={`font-bold text-xs ${isCustom ? "text-purple-100" : isDark ? "text-slate-100" : "text-slate-900"}`}>
                  {language === "bn" ? "ডেভেলপার ও কারিগরি সহায়তা" : "Developer & Technical Support"}
                </h4>
                <p className="text-xs opacity-70">
                  {language === "bn" ? "সফটওয়্যার উন্নয়ন, বাস্তবায়ন ও রক্ষণাবেক্ষণ" : "Software Engineering, Implementation & Maintenance"}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2 pt-1 text-xs">
              <div className={`flex items-center justify-between border-b pb-1.5 ${
                isCustom ? "border-[#36275d]" : isDark ? "border-slate-700/60" : "border-emerald-200/80"
              }`}>
                <span className="opacity-75">{language === "bn" ? "প্রকৌশল দল:" : "Engineering:"}</span>
                <span className={`font-bold ${isCustom ? "text-amber-300" : isDark ? "text-emerald-400" : "text-emerald-800"}`}>
                  PJC Automation
                </span>
              </div>
              ...
              <div className={`flex items-center justify-between border-b pb-1.5 ${
                ...
                <a
                  ...
                  className={`font-mono font-bold hover:underline ${
                    isCustom ? "text-amber-300" : isDark ? "text-sky-400" : "text-emerald-700"
                  }`}
                >
                  +8801820302571
                </a>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-medium">Rangamati, Chattogram, Bangladesh</span>
              </div>
            </div>
          </div>
