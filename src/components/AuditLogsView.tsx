...
export function AuditLogsView({ auditLogs, users }: AuditLogsViewProps) {
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-slate-900">Audit Logs & History Trail</h2>
        <p className="text-sm text-slate-500">Immutable history tracking for all financial transactions, allocations, and approvals.</p>
      </div>
              ...
              auditLogs.map((log) => {
                ...
                return (
                  <tr key={log.id} className="hover:bg-slate-50/60 transition">
                    <td className="p-4 text-xs font-mono text-slate-600">{log.timestamp}</td>
                    ...
                    <td className="p-4 text-xs font-semibold text-emerald-700">{log.tableName}</td>
