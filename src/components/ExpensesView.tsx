...
import { Expense, Allocation, Office, Category, FinancialYear, User, NoteSheet, ApplicantInfo, EntryOfficerInfo, QuotationItem, BranchDebitEntry } from "../types";
import { Receipt, Plus, Building2, Building, Users, FileText, Trash2, AlertTriangle, AlertCircle, UserCircle, Loader2, Printer, Edit, CheckCircle, XCircle, Clock, Calendar, Check, X, Paperclip, Eye, RefreshCw, Sparkles } from "lucide-react";
...
export function ExpensesView({
  ...
  const [internalStatusFilter, setInternalStatusFilter] = useState<"All" | "Pending" | "Approved" | "Rejected">("All");

  const currentStatusFilter = propStatusFilter !== undefined ? propStatusFilter : internalStatusFilter;
  const setStatusFilter = propSetStatusFilter || setInternalStatusFilter;

  const [showModal, setShowModal] = useState(false);
  const [editingExpenseId, setEditingExpenseId] = useState<string | null>(null);
  const [deletingId, setDeletingId] = useState<string | null>(null);
  const [previewNoteSheet, setPreviewNoteSheet] = useState<NoteSheet | null>(null);
  ...
  const [officeId, setOfficeId] = useState(isHeadOffice ? (offices[0]?.id || "") : currentUser.officeId);
  ...
  const [expenseType, setExpenseType] = useState<"General" | "Quotation">("General");
  const [quotationFormType, setQuotationFormType] = useState<"Form1" | "Form2">("Form1");
  const [memoForwardingNo, setMemoForwardingNo] = useState("সূত্র নং-কশি-০৬ (অংশ-০৩)/২০২৬-২০২৭/");
  const [memoSupplyOrderNo, setMemoSupplyOrderNo] = useState("সূত্র নং-প্রশ-১(৪০)/২০২৬-২০২৭/");
  ...
  const [debitAccount, setDebitAccount] = useState(categories[0]?.code || "১৩৪/০৫");
  ...
  const [supplyRecipientName, setSupplyRecipientName] = useState("জনাব সুমন বিকাশ চাকমা");
  const [supplyRecipientDesignation, setSupplyRecipientDesignation] = useState("সিইও");
  const [supplyRecipientOrgName, setSupplyRecipientOrgName] = useState("কম্পিউটার ভিলেজ");
  const [supplyRecipientAddress1, setSupplyRecipientAddress1] = useState("বনরূপা");
  const [supplyRecipientAddress2, setSupplyRecipientAddress2] = useState("রাঙ্গামাটি।");
  ...
  const [form2Items, setForm2Items] = useState<Array<{
  ...
  const [quotationItems, setQuotationItems] = useState<QuotationItem[]>([
    {
      ...
      remarks: "",
      suppliers: [
        { nameAndAddress: "", unitPrice: 0, qty: 1, totalPrice: 0, vatRate: 0, taxRate: 0, remarks: "" }
      ]
    }
  ]);
  ...
  // Applicant Info
  const [applicantType, setApplicantType] = useState<"OwnOffice" | "PersonInstitution">("OwnOffice");
  ...
  const availableCategories = expenseType === "Quotation"
    ? categories.filter(c => (c.status === "Active" && c.allowInQuotation !== false) || (editingExpenseId && c.id === categoryId))
    : categories.filter(c => c.status === "Active" || (editingExpenseId && c.id === categoryId));
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async () => {
      try {
        ...
        if (!res.ok || !data.url) {
          setUploadError(
            data.error || (language === "bn" ? "ফাইল আপলোড ব্যর্থ হয়েছে।" : "File upload failed.")
          );
        } else {
  ...
  const handleEditClick = (exp: Expense) => {
    ...
    if (exp.hasStockChalan) {
      ...
      setHasStockChalan(exp.taxRate === 0 || exp.taxRate === undefined ? "হ্যাঁ" : "না");
    } else {
      setHasStockChalan("হ্যাঁ");
    }
    ...
    if (exp.supplyRecipientName) setSupplyRecipientName(exp.supplyRecipientName);
    if (exp.supplyRecipientDesignation) setSupplyRecipientDesignation(exp.supplyRecipientDesignation);
    if (exp.supplyRecipientOrgName) setSupplyRecipientOrgName(exp.supplyRecipientOrgName);
    if (exp.supplyRecipientAddress1) setSupplyRecipientAddress1(exp.supplyRecipientAddress1);
    if (exp.supplyRecipientAddress2) setSupplyRecipientAddress2(exp.supplyRecipientAddress2);
    ...
    if (exp.quotationItems && exp.quotationItems.length > 0) {
      if (exp.quotationFormType === "Form2") {
        const loadedForm2Items = exp.quotationItems.map(qi => ({
          ...
          qty: qi.qty || 1,
          unit: qi.unit || "টি",
          suppliers: (qi.suppliers && qi.suppliers.length >= 3)
            ? qi.suppliers.slice(0, 3).map((s, idx) => ({
                orgIndex: idx + 1,
                orgName: s.nameAndAddress || (idx === 0 ? exp.supplierOrg1 || defaultSupplierOrg1 : idx === 1 ? exp.supplierOrg2 || defaultSupplierOrg2 : exp.supplierOrg3 || defaultSupplierOrg3),
                unitPrice: s.unitPrice || 0,
                qty: s.qty || qi.qty || 1,
                totalPrice: s.totalPrice || ((s.unitPrice || 0) * (s.qty || qi.qty || 1)),
                remarks: s.remarks || (idx === 0 ? "সর্বনিম্ন দরদাতা" : idx === 1 ? "সর্বোচ্চ দরদাতা" : "২য় সর্বোচ্চ দরদাতা")
              }))
            : [
                { orgIndex: 1, orgName: exp.supplierOrg1 || defaultSupplierOrg1, unitPrice: qi.unitPrice || 0, qty: qi.qty || 1, totalPrice: qi.totalPrice || 0, remarks: "সর্বনিম্ন দরদাতা" },
                { orgIndex: 2, orgName: exp.supplierOrg2 || defaultSupplierOrg2, unitPrice: 0, qty: qi.qty || 1, totalPrice: 0, remarks: "সর্বোচ্চ দরদাতা" },
                { orgIndex: 3, orgName: exp.supplierOrg3 || defaultSupplierOrg3, unitPrice: 0, qty: qi.qty || 1, totalPrice: 0, remarks: "২য় সর্বোচ্চ দরদাতা" }
              ]
        }));
      ...
      const defaultItems: QuotationItem[] = [
        {
          ...
          remarks: "সর্বনিম্ন দরদাতা",
          suppliers: [
            { nameAndAddress: exp.supplierOrg1 || defaultSupplierOrg1, unitPrice: Number(exp.amount || exp.baseAmount || 0), qty: 1, totalPrice: Number(exp.amount || exp.baseAmount || 0), vatRate: exp.vatRate || 0, taxRate: exp.taxRate || 0, remarks: "সর্বনিম্ন দরদাতা" },
            { nameAndAddress: exp.supplierOrg2 || defaultSupplierOrg2, unitPrice: Math.round(Number(exp.amount || exp.baseAmount || 0) * 1.08), qty: 1, totalPrice: Math.round(Number(exp.amount || exp.baseAmount || 0) * 1.08), vatRate: exp.vatRate || 0, taxRate: exp.taxRate || 0, remarks: "২য় সর্বোচ্চ দরদাতা" },
            { nameAndAddress: exp.supplierOrg3 || defaultSupplierOrg3, unitPrice: Math.round(Number(exp.amount || exp.baseAmount || 0) * 1.12), qty: 1, totalPrice: Math.round(Number(exp.amount || exp.baseAmount || 0) * 1.12), vatRate: exp.vatRate || 0, taxRate: exp.taxRate || 0, remarks: "সর্বোচ্চ দরদাতা" }
          ]
        }
      ];
      if (exp.quotationFormType === "Form2") {
        setForm2Items(defaultItems.map(qi => ({
          itemDescription: qi.itemDescription,
          qty: qi.qty || 1,
          unit: qi.unit || "টি",
    ...
    setBranchEntries(exp.branchEntries && Array.isArray(exp.branchEntries) ? exp.branchEntries : []);
  ...
  const parseOrgAddress = (raw: string) => {
    ...
    if (parts.length <= 1) {
      ...
      const address1 = parts.slice(Math.min(2, parts.length - 2), parts.length - 1).join(", ");
  ...
  const suggestedSuppliers = useMemo(() => {
    ...
    expenses.forEach(exp => {
      ...
      if (exp.supplyRecipientOrgName?.trim()) set.add(exp.supplyRecipientOrgName.trim());
  ...
  }, [expenses, quotationItems, supplierOrg1, supplierOrg2, supplierOrg3, form2Items]);

  // Form-2 Lowest Bidder & Computed Totals
  const form2LowestBidder = useMemo(() => {
    const org1Total = form2Items.reduce((acc, item) => acc + (Number(item.suppliers[0]?.totalPrice) || 0), 0);
    const org2Total = form2Items.reduce((acc, item) => acc + (Number(item.suppliers[1]?.totalPrice) || 0), 0);
    const org3Total = form2Items.reduce((acc, item) => acc + (Number(item.suppliers[2]?.totalPrice) || 0), 0);
    ...
    const minOrg = withPrice.reduce((min, cur) => cur.total < min.total ? cur : min, withPrice[0]);
    return { ...minOrg, parsed: parseOrgAddress(minOrg.name) };
  }, [form2Items, supplierOrg1, supplierOrg2, supplierOrg3]);

  const effectiveTaxRate = (expenseType === "Quotation" && hasStockChalan === "হ্যাঁ") ? 0 : (Number(taxRate) || 0);
  const effectiveVatRate = Number(vatRate) || 0;

  const form1LowestBaseTotal = useMemo(() => {
    return quotationItems.reduce((acc, item) => acc + (item.totalPrice || 0), 0);
  }, [quotationItems]);
  ...
  const form2LowestBaseTotal = useMemo(() => {
    return form2LowestBidder.total || form2Items.reduce((acc, item) => acc + (Number(item.suppliers[0]?.totalPrice) || 0), 0);
  ...
  }, [expenseType, quotationFormType, form1GrandTotal, form2GrandTotal, amount]);

  // Form-2 Handlers
  const handleForm2SupplierNameChange = (orgIdx: 1 | 2 | 3, newName: string) => {
    if (orgIdx === 1) setSupplierOrg1(newName);
    if (orgIdx === 2) setSupplierOrg2(newName);
    if (orgIdx === 3) setSupplierOrg3(newName);

    setForm2Items(prev => prev.map(item => {
  ...
  const handleForm2PriceChange = (itemIdx: number, supIdx: number, price: number) => {
    setForm2Items(prev => {
      const updated = [...prev];
      ...
      suppliers[supIdx].totalPrice = Number((price * targetItem.qty).toFixed(2));
      ...
      suppliers.forEach((s, idx) => {
        ...
        } else if (s.unitPrice === maxPrice && maxPrice > 0 && positivePrices.length > 1) {
  ...
  const handleAddForm2Item = () => {
    setForm2Items(prev => [
      ...prev,
      {
        ...
        unit: "টি",
        suppliers: [
          { orgIndex: 1, orgName: supplierOrg1, unitPrice: 0, qty: 1, totalPrice: 0, remarks: "" },
          { orgIndex: 2, orgName: supplierOrg2, unitPrice: 0, qty: 1, totalPrice: 0, remarks: "" },
          { orgIndex: 3, orgName: supplierOrg3, unitPrice: 0, qty: 1, totalPrice: 0, remarks: "" },
        ]
      }
    ]);
  };
  ...
  const filteredExpenses = expenses.filter(e => {
    ...
    const matchOffice = isHeadOffice ? true : e.officeId === currentUser.officeId;
  ...
  const editingExpenseObj = editingExpenseId ? expenses.find(e => e.id === editingExpenseId) : null;
  ...
  const categoryAllocations = allocations.filter(
    a => a.financialYearId === activeModalFY && a.officeId === officeId && a.categoryId === categoryId
  );
  ...
  const catExpenses = expenses.filter(
    ...
    e.financialYearId === activeModalFY && e.officeId === officeId && e.categoryId === categoryId
  );
  ...
  const isNegativeBalance = !currentCategory?.allowExcess && effectiveCurrentAmount > availableBalance;
  ...
  const handleGenerateNoteSheet = async (expenseId: string) => {
    ...
    try {
      const res = await apiFetch(`/api/expenses/${expenseId}/generate-notesheet`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (data.success) {
        ...
        showToast(language === "bn" ? "নোট শিট তৈরি হয়েছে" : "Note Sheet generated.");
      } else {
  ...
  const handleBulkGenerate = async () => {
    ...
    if (pendingExpenses.length === 0) {
      alert(language === "bn" ? "কোনো পেন্ডিং নোট শিট নেই।" : "No pending note sheets to generate.");
      return;
    }
    ...
    for (const exp of pendingExpenses) {
      try {
        const res = await apiFetch(`/api/expenses/${exp.id}/generate-notesheet`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    if (successCount > 0) {
      if (refreshData) refreshData();
      showToast(language === "bn" ? `${successCount} টি নোট শিট তৈরি হয়েছে!` : `${successCount} Note Sheets generated!`);
    } else {
      showToast(language === "bn" ? "নোট শিট তৈরি করা যায়নি। অনুগ্রহ করে সংশ্লিষ্ট খাতের টেমপ্লেট কনফিগার করুন।" : "Failed to generate note sheets. Ensure templates are configured.");
    }
  };
  ...
  const handleApproveAction = async (id: string) => {
    ...
    try {
      ...
      showToast(language === "bn" ? "ব্যয় অনুমোদিত হয়েছে।" : "Expense approved.");
    } catch (err: any) {
  ...
  const handleRejectConfirm = async () => {
    ...
    if (!rejectionReasonInput.trim()) {
      setRejectError(language === "bn" ? "প্রত্যাখ্যানের কারণ উল্লেখ করা আবশ্যক।" : "Rejection reason is required.");
      return;
    }
    try {
      setRejectError("");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (currentFYObj?.startDate && currentFYObj?.endDate && expenseDate) {
      if (expenseDate < currentFYObj.startDate || expenseDate > currentFYObj.endDate) {
    ...
    const form1BaseTotal = quotationItems.reduce((acc, item) => acc + (item.totalPrice || 0), 0);
    ...
    const effectiveAmount = (expenseType === "Quotation" && quotationFormType === "Form1")
      ? form1GrandTotal
      : (expenseType === "Quotation" && quotationFormType === "Form2")
      ? form2GrandTotal
      : Number(amount);

    const finalDescription = (expenseType === "Quotation" && quotationFormType === "Form1")
      ? (quotationItems[0]?.itemDescription || "Quotation Expense")
      : (expenseType === "Quotation" && quotationFormType === "Form2")
      ? (form2Items.map(i => `${i.itemDescription} (${i.qty} ${i.unit})`).filter(Boolean).join(", ") || "কোটেশন ব্যয় (ফর্ম-২)")
      : description;

    if (!effectiveAmount || !voucherNo || (!finalDescription && expenseType !== "Quotation") || isDuplicateVoucher || isNegativeBalance) return;

    const applicant: ApplicantInfo = expenseType === "Quotation"
      ? {
          type: "OwnOffice",
      ...
      : {
          ...
          designation: applicantDesignation || currentUser.designation || "Officer",
          officeId: applicantType === "OwnOffice" ? officeId : undefined,
          institutionName: applicantType === "PersonInstitution" ? applicantInstitution : undefined
        };
    ...
    const defaultStatus = categoryObj?.requireApproval === false ? "Approved" : "Pending";

    const serializedQuotationItems = (expenseType === "Quotation" && quotationFormType === "Form1")
    ...
    const quotationPayload = expenseType === "Quotation" ? {
      baseAmount: quotationFormType === "Form1" ? form1BaseTotal : form2LowestBaseTotal,
      vatRate: vatRate ? Number(vatRate) : undefined,
      vatAmount: quotationFormType === "Form1" ? form1VatAmt : form2VatAmount,
      taxRate: effectiveTaxRate,
      taxAmount: quotationFormType === "Form1" ? form1TaxAmt : form2TaxAmount,
      hasStockChalan: hasStockChalan,
      vatChalanNo: hasStockChalan === "হ্যাঁ" ? vatChalanNo : undefined,
      vatChalanDate: hasStockChalan === "হ্যাঁ" ? vatChalanDate : undefined,
    } : {
    ...
    try {
      if (editingExpenseId && onUpdateExpense) {
        await onUpdateExpense(editingExpenseId, {
          ...
          categoryId,
          expenseType,
          quotationFormType: expenseType === "Quotation" ? quotationFormType : undefined,
          expenseDate,
          ...
          branchEntries: (expenseType === "Quotation" && quotationFormType === "Form1") ? branchEntries : [],
          debitAccount,
          paymentType: "একক সরবরাহকারী",
          memoForwardingNo: expenseType === "Quotation" ? memoForwardingNo : undefined,
          memoSupplyOrderNo: expenseType === "Quotation" ? memoSupplyOrderNo : undefined,
          quotationDate: expenseType === "Quotation" ? quotationDate : undefined,
          supplyRecipientName: expenseType === "Quotation" ? supplyRecipientName : undefined,
          supplyRecipientDesignation: expenseType === "Quotation" ? supplyRecipientDesignation : undefined,
          supplyRecipientOrgName: expenseType === "Quotation" 
            ? (supplyRecipientOrgName || (quotationFormType === "Form1" ? form1LowestBidder.name : form2LowestBidder.name) || supplierOrg1) 
            : undefined,
          supplyRecipientAddress1: expenseType === "Quotation" 
            ? (supplyRecipientAddress1 || (quotationFormType === "Form1" ? form1LowestBidder.parsed.address1 : form2LowestBidder.parsed.address1) || "নজির আহমেদ চৌধুরী রোড") 
            : undefined,
          supplyRecipientAddress2: expenseType === "Quotation" 
            ? (supplyRecipientAddress2 || (quotationFormType === "Form1" ? form1LowestBidder.parsed.address2 : form2LowestBidder.parsed.address2) || "আন্দরকিল্লা, চট্টগ্রাম।") 
            : undefined,
      ...
      setShowModal(false);
      if (refreshData) refreshData();
      showToast(language === "bn" ? "ব্যয় সফলভাবে যুক্ত/হালনাগাদ হয়েছে!" : "Expense saved successfully!");
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className={`text-xl font-bold ${isCustom ? "text-amber-400" : isDark ? "text-slate-100" : "text-slate-900"}`}>
            {t.expensesTitle}
          </h2>
          <p className={`text-xs mt-0.5 ${isCustom ? "text-purple-300" : isDark ? "text-slate-400" : "text-slate-500"}`}>
      ...
      {isFYClosed && (
        <div className="p-3.5 bg-amber-500/10 border border-amber-500/30 text-amber-300 rounded-2xl text-xs font-semibold flex items-center gap-2">
          ...
          <span>🔒 নির্বাচিত অর্থবছর ({currentFYObj?.name}) বন্ধ (Closed) করা হয়েছে। নতুন ব্যয় ভাউচার যোগ বা পরিবর্তন করা সম্পূর্ণ বন্ধ রয়েছে।</span>
        </div>
      )}

      {/* Status Filter Tabs */}
      <div className={`flex items-center gap-2 border-b pb-3 overflow-x-auto ${
        isCustom ? "border-[#382b61]" : isDark ? "border-slate-800" : "border-slate-200"
      }`}>
        {(["All", "Pending", "Approved", "Rejected"] as const).map(st => {
          ...
          let badgeColor = isCustom ? "bg-[#251d45] text-purple-300" : isDark ? "bg-slate-800 text-slate-400" : "bg-slate-100 text-slate-600";
          if (st === "Pending") badgeColor = "bg-amber-500/15 text-amber-400";
          if (st === "Approved") badgeColor = "bg-emerald-500/15 text-emerald-400";
          ...
          return (
            ...
            >
              <span>
                {st === "All" && (language === "bn" ? "সকল ব্যয়" : "All Expenses")}
                {st === "Pending" && (language === "bn" ? "অপেক্ষমান (Pending)" : "Pending")}
                {st === "Approved" && (language === "bn" ? "অনুমোদিত (Approved)" : "Approved")}
                {st === "Rejected" && (language === "bn" ? "বাতিল (Rejected)" : "Rejected")}
              </span>
              <span className={`px-1.5 py-0.5 rounded-full text-xs font-bold ${isActive ? (isCustom ? "bg-[#3d2b75] text-amber-200" : isDark ? "bg-slate-700 text-white" : "bg-slate-800 text-white") : badgeColor}`}>
                {count}
              </span>
            </button>
          );
      ...
      <div className={`rounded-2xl shadow-sm border overflow-hidden ${
        isCustom ? "bg-[#18122d] border-[#382b61]" : isDark ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"
      }`}>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className={`border-b text-xs uppercase tracking-wider ${
                ...
                <th className="p-3.5 font-semibold">{language === "bn" ? "ব্যয়ের তারিখ ও পত্রে বিবরণ" : "Expense Date & Voucher"}</th>
                <th className="p-3.5 font-semibold">{t.office} & {t.category}</th>
                <th className="p-3.5 font-semibold">{t.applicantName}</th>
                <th className="p-3.5 font-semibold">{t.description}</th>
                <th className="p-3.5 font-semibold text-center">{language === "bn" ? "অনুমোদন স্ট্যাটাস" : "Approval Status"}</th>
                <th className="p-3.5 font-semibold text-right">{t.amount}</th>
                <th className="p-3.5 font-semibold text-right">{t.actions}</th>
              </tr>
            </thead>
            <tbody className={`divide-y text-xs ${
              isCustom ? "divide-[#281d47]" : isDark ? "divide-slate-800" : "divide-slate-100"
            }`}>
              {filteredExpenses.length === 0 ? (
                ...
                filteredExpenses.map((exp) => {
                  ...
                  return (
                    <tr key={exp.id} className={`transition group ${
                      isCustom ? "hover:bg-[#20183d]" : isDark ? "hover:bg-slate-800/60" : "hover:bg-slate-50/70"
                    }`}>
                      <td className="p-3.5">
                        <div className={`font-mono font-bold flex items-center gap-1.5 ${
                          isCustom ? "text-amber-300" : isDark ? "text-slate-100" : "text-slate-900"
                        }`}>
                          <Receipt className="w-3.5 h-3.5 opacity-50 shrink-0" />
                          {exp.voucherNo}
                        </div>
                        <div className={`text-xs mt-1 flex items-center gap-1 font-mono opacity-70`}>
                      ...
                      <td className="p-3.5">
                        <div className={`font-medium flex items-center gap-1 ${
                          isCustom ? "text-purple-100" : isDark ? "text-slate-200" : "text-slate-800"
                        }`}>
                          <Building2 className="w-3 h-3 opacity-60" /> {off?.name}
                        </div>
                      ...
                      <td className="p-3.5 text-xs">
                        <div className={`font-medium ${isCustom ? "text-purple-100" : isDark ? "text-slate-200" : "text-slate-800"}`}>{exp.applicant?.name || "-"}</div>
                        <div className={`text-xs ${isCustom ? "text-purple-300" : isDark ? "text-slate-400" : "text-slate-500"}`}>{exp.applicant?.designation || ""}</div>
                        {exp.applicant?.type === "PersonInstitution" && (
                          <div className="text-xs opacity-60 mt-0.5 italic">{exp.applicant?.institutionName}</div>
                        )}
                      </td>
                      <td className="p-3.5 text-xs max-w-xs">
                        <div className={`font-medium mb-0.5 flex items-center gap-1 ${
                          ...
                          {exp.expenseType === "Quotation" && (
                            <span className="inline-block px-1.5 py-0.5 bg-blue-500/20 text-blue-300 border border-blue-500/30 rounded text-xs font-bold">QTN</span>
                          )}
                          {exp.description}
                        </div>
                        {exp.entryOfficer?.name && (
                          <div className={`mt-1.5 text-xs font-mono opacity-60`}>
                            <span>{t.entryOfficer}: {exp.entryOfficer.name}</span>
                          </div>
                        )}
                        {exp.supportingDocument && (
                          <div className="mt-1.5">
                            <a
                              ...
                              title={language === "bn" ? "সংযুক্তি দেখুন" : "View Supporting Document"}
                            >
                      ...
                      <td className="p-3.5 text-center">
                        ...
                        {exp.status === "Rejected" && (
                          <div className="inline-flex flex-col items-center">
                            ...
                            {exp.rejectionReason && (
                              <span className="text-xs text-rose-400 mt-1 max-w-[140px] truncate" title={exp.rejectionReason}>
                                {exp.rejectionReason}
                              </span>
                            )}
                          </div>
                      ...
                      <td className={`p-3.5 font-bold text-right font-mono text-sm ${
                        isCustom ? "text-amber-300 bg-[#140e29]/40" : isDark ? "text-slate-100 bg-slate-800/30" : "text-slate-900 bg-slate-50/30"
                      }`}>
                      ...
                      <td className="p-3.5 text-right space-x-1">
                        ...
                        {isHeadOffice && exp.status === "Pending" && (
                          <>
                            <button
                              ...
                              title={language === "bn" ? "ব্যয় অনুমোদন করুন" : "Approve Expense"}
                            >
                            ...
                            <button
                              ...
                              title={language === "bn" ? "ব্যয় বাতিল করুন" : "Reject Expense"}
                            >
                              <X className="w-4 h-4" />
                            </button>
                          </>
                        ...
                        <button
                          ...
                          title={language === "bn" ? "সম্পাদনা করুন" : "Edit Expense"}
                        >
                        ...
                        {exp.noteSheetId ? (
                          <>
                            <button
                              onClick={() => {
                                const ns = noteSheets.find(n => n.id === exp.noteSheetId || n.expenseId === exp.id);
                            ...
                            <button
                              ...
                              title={language === "bn" ? "ব্যয় অনুযায়ী নোটশিট রিফ্রেশ / পুনর্নির্মাণ করুন" : "Re-sync Note Sheet from current expense"}
                            >
                              {generatingIds.has(exp.id) ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
                            </button>
                          </>
                        ) : (
                          <button
                          ...
                          >
                            {generatingIds.has(exp.id) ? <Loader2 className="w-4 h-4 animate-spin" /> : <Printer className="w-4 h-4" />}
                          </button>
                        )}
                        <button
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-start justify-center p-4 overflow-y-auto">
          <div className={`rounded-2xl max-w-4xl w-full p-6 shadow-xl border m-auto ${
            isCustom ? "bg-[#18122d] border-[#382b61] text-purple-100" : isDark ? "bg-slate-900 border-slate-700 text-slate-100" : "bg-white border-slate-200 text-slate-900"
          }`}>
            <div className={`flex justify-between items-center pb-3 border-b mb-4 ${
              isCustom ? "border-[#2e234e]" : isDark ? "border-slate-800" : "border-slate-200"
            }`}>
              <h3 className="text-base font-bold">
                {editingExpenseId
                  ? (language === "bn" ? "ব্যয় সংশোধনের বিবরণ" : "Edit Expense Entry")
                  : t.addExpense}
              </h3>
              <button 
                type="button" 
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              ...
              <div className={`flex gap-2 mb-2 p-1 rounded-xl ${
                isCustom ? "bg-[#120d24]" : isDark ? "bg-slate-800" : "bg-slate-100"
              }`}>
                <button 
                  type="button"
                  onClick={() => {
                    setExpenseType("General");
                    const valid = categories.filter(c => c.status === "Active");
                ...
                <button 
                  ...
                  onClick={() => {
                    ...
                    const valid = categories.filter(c => c.status === "Active" && c.allowInQuotation !== false);
                    if (valid.length > 0 && !valid.some(c => c.id === categoryId)) {
                      setCategoryId(valid[0].id);
                    }
                  }}
                ...
                >
                  {language === "bn" ? "কোটেশন প্রক্রিয়ায় ব্যয়" : "Quotation Expense"}
                </button>
              </div>

              {/* Expense & Voucher Dates */}
              <div className={`p-4 rounded-xl border space-y-3 ${
                isCustom ? "bg-[#140e29] border-[#382b61]" : isDark ? "bg-slate-800/70 border-slate-700" : "bg-slate-50 border-slate-200"
              }`}>
                <div className={`flex items-center gap-1.5 font-bold ${
                  isCustom ? "text-amber-300" : isDark ? "text-slate-200" : "text-slate-800"
                }`}>
                  <Receipt className="w-4 h-4 text-emerald-400" /> {language === "bn" ? "ব্যয় ও পত্রের বিবরণ" : "Expense & Voucher Info"}
                </div>
                
                {isHeadOffice && (
                  <div>
                    <label className={`block font-semibold mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>{t.office}</label>
                    <select
                      value={officeId}
                      onChange={e => setOfficeId(e.target.value)}
                      className={`w-full px-3 py-2 border rounded-xl focus:outline-none ${
                        isCustom ? "bg-[#18122d] border-[#382b61] text-purple-100" : isDark ? "bg-slate-850 border-slate-700 text-slate-100" : "bg-white border-slate-300 text-slate-900"
                      }`}
                    >
                      {offices.map(o => (
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className={`block font-semibold mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>{language === "bn" ? "ব্যয়ের তারিখ (Expense Date)" : "Expense Date"} *</label>
                    <input
                      type="date"
                      required
                      value={expenseDate}
                  ...
                  <div>
                    <label className={`block font-semibold mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>{language === "bn" ? "পত্রের নম্বর (Voucher No)" : "Patra No"} *</label>
                    <input
                      type="text"
                      required
                      value={voucherNo}
                    ...
                    />
                    {isDuplicateVoucher && (
                      <p className="text-xs text-rose-400 mt-1 flex items-center gap-1"><AlertTriangle className="w-3 h-3"/> {t.duplicateVoucherAlert}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className={`block font-semibold mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>{language === "bn" ? "পত্রের তারিখ (Voucher Date)" : "Patra Date"} *</label>
                    <input
                      type="date"
                      required
                      value={voucherDate}
                  ...
                  <div>
                    <label className={`block font-semibold mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>{t.category} *</label>
                    <select
                      value={categoryId}
                      onChange={e => {
                        ...
                        if (selCat) {
                          setDebitAccount(selCat.code || selCat.budgetHead || selCat.name);
                        }
                      }}
                      className={`w-full px-3 py-2 border rounded-xl focus:outline-none ${
                        isCustom ? "bg-[#18122d] border-[#382b61] text-purple-100" : isDark ? "bg-slate-850 border-slate-700 text-slate-100" : "bg-white border-slate-300 text-slate-900"
                    ...
                    >
                      {availableCategories.map(cat => (
                        <option key={cat.id} value={cat.id}>{cat.name} ({cat.budgetHead})</option>
                      ))}
                    </select>
                  </div>
                </div>
                ...
                {expenseType === "Quotation" && (
                  <div className="space-y-3 pt-2 border-t border-emerald-500/20">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className={`block font-semibold mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>
                          {language === "bn" ? "কোটেশন ফর্ম স্টাইল" : "Quotation Form Style"}
                        </label>
                        <select
                          value={quotationFormType}
                          onChange={e => setQuotationFormType(e.target.value as "Form1" | "Form2")}
                          className={`w-full px-3 py-2 border rounded-xl focus:outline-none font-semibold ${
                            isCustom ? "bg-[#18122d] border-[#382b61] text-amber-300" : isDark ? "bg-slate-850 border-slate-700 text-emerald-400" : "bg-emerald-50 border-emerald-300 text-emerald-800"
                          }`}
                        >
                          <option value="Form1">FORM–1 (Combination Form)</option>
                          <option value="Form2">FORM–2 (দরপত্র তুলনামূলক বিবরণী)</option>
                        </select>
                      </div>
                      <div>
                        <label className={`block font-semibold mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>
                          {language === "bn" ? "মেমো/সূত্র নম্বর (ফরোয়ার্ডিং)" : "Memo / Forwarding No"}
                        </label>
                        <input
                          type="text"
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className={`block font-semibold mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>
                            {language === "bn" ? "মেমো/সূত্র নম্বর (সাপ্লাই অর্ডার)" : "Supply Order Memo No"}
                          </label>
                          <input
                            type="text"
                        ...
                        <div>
                          <label className={`block font-semibold mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>
                            {language === "bn" ? "কোটেশন তারিখ (কার্যাদেশে ব্যবহার)" : "Quotation Date"}
                          </label>
                          <input
                            type="date"
                            value={quotationDate}
                ...
                {expenseType === "Quotation" && (
                  <div className={`p-3.5 rounded-xl border ${
                    isCustom ? "bg-[#140e29] border-[#382b61]" : isDark ? "bg-slate-800/70 border-slate-700" : "bg-emerald-50/60 border-emerald-200"
                  }`}>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      <label className={`block font-semibold text-xs ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-800"}`}>
                        {language === "bn" ? "মুসক চালান আছে কি না?" : "Has Musok Chalan?"}
                      </label>
                      <div className="flex items-center gap-4">
                        <label className="inline-flex items-center gap-1.5 cursor-pointer text-xs">
                          ...
                          <span className="font-semibold text-emerald-700 dark:text-emerald-400">
                            {language === "bn" ? "হ্যাঁ (ট্যাক্স ০%)" : "Yes (Tax 0%)"}
                          </span>
                        </label>
                        <label className="inline-flex items-center gap-1.5 cursor-pointer text-xs">
                          <input
                            ...
                            onChange={() => {
                              ...
                              if (taxRate === "0" || !taxRate) setTaxRate("3.0");
                            }}
                            className="text-emerald-600 focus:ring-emerald-500"
                          />
                          <span className="font-semibold text-slate-700 dark:text-slate-300">
                            {language === "bn" ? "না (ট্যাক্স প্রযোজ্য)" : "No (Tax Applicable)"}
                          </span>
                        </label>
                      </div>
                    </div>
                    {hasStockChalan === "হ্যাঁ" ? (
                      <div className="mt-3 pt-3 border-t border-emerald-500/30 space-y-3">
                        <p className="text-[11px] font-medium text-emerald-700 dark:text-emerald-400">
                          {language === "bn" ? "ℹ️ মুসক চালান থাকায় ট্যাক্স কাউন্ট হবে না (ট্যাক্স হার স্বয়ংক্রিয়ভাবে ০% নির্ধারিত)।" : "ℹ️ Tax will not be counted because Musok Chalan is Yes (Tax set to 0%)."}
                        </p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <div>
                            <label className={`block font-semibold text-xs mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>
                              {language === "bn" ? "মুসক চালান নম্বর" : "VAT Chalan No"}
                            </label>
                            <input
                              type="text"
                              value={vatChalanNo}
                          ...
                          <div>
                            <label className={`block font-semibold text-xs mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>
                              {language === "bn" ? "মুসক চালান তারিখ" : "VAT Chalan Date"}
                            </label>
                            <input
                              type="date"
                              value={vatChalanDate}
                      ...
                      <p className="text-[11px] font-medium text-amber-700 dark:text-amber-400 mt-1.5">
                        {language === "bn" ? "ℹ️ মুসক চালান না থাকায় প্রযোজ্য ট্যাক্স হার (%) ইনপুট দিন।" : "ℹ️ Musok Chalan is No. Please enter applicable Tax rate (%)."}
                      </p>
                    )}
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className={`block font-semibold mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>{language === "bn" ? "ভ্যাট হার (%)" : "VAT Rate (%)"}</label>
                    <input
                      type="number"
                      min="0"
                      step="0.1"
                      ...
                      placeholder="e.g. 7.5"
                      className={`w-full px-3 py-2 border rounded-xl focus:outline-none ${
                        isCustom ? "bg-[#18122d] border-[#382b61] text-purple-100 placeholder:text-purple-400/50" : isDark ? "bg-slate-850 border-slate-700 text-slate-100 placeholder:text-slate-500" : "bg-white border-slate-300 text-slate-900"
                      }`}
                    />
                  </div>
                  <div>
                    <label className={`block font-semibold mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>
                      ...
                      {expenseType === "Quotation" && hasStockChalan === "হ্যাঁ" && (
                        <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-bold ml-1.5">(মুসক চালানে ০%)</span>
                      )}
                    </label>
                    <input
                      type="number"
                      min="0"
                      step="0.1"
                      value={expenseType === "Quotation" && hasStockChalan === "হ্যাঁ" ? "0" : taxRate}
                      disabled={expenseType === "Quotation" && hasStockChalan === "হ্যাঁ"}
                      onChange={e => setTaxRate(e.target.value)}
                      placeholder={expenseType === "Quotation" && hasStockChalan === "হ্যাঁ" ? "0" : "e.g. 3.0"}
                      className={`w-full px-3 py-2 border rounded-xl focus:outline-none ${
                        expenseType === "Quotation" && hasStockChalan === "হ্যাঁ"
                ...
                <div>
                  <label className={`block font-semibold mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>
                    {language === "bn" ? "মূল পরিমাণ (Base Amount)" : "Base Amount"} ({language === "bn" ? "টাকা" : "BDT"}) *
                  </label>
                  <input
                    type="number"
                    required
                    ...
                    value={
                      expenseType === "Quotation" && quotationFormType === "Form1"
                        ? quotationItems.reduce((acc, item) => acc + (item.totalPrice || 0), 0)
                        : expenseType === "Quotation" && quotationFormType === "Form2"
                    ...
                    className={`w-full px-3 py-2 border rounded-xl focus:outline-none ${
                      expenseType === "Quotation" ? "bg-emerald-500/10 font-bold font-mono text-emerald-600 dark:text-emerald-400" : ""
                    } ${
                  ...
                  {expenseType === "Quotation" && (
                    <p className="text-[10px] text-emerald-600 dark:text-emerald-400 mt-1 font-medium">
                      {language === "bn" ? "ℹ️ মূল পরিমাণ নিচে প্রদত্ত পণ্য/আইটেম বিবরণ থেকে স্বয়ংক্রিয়ভাবে হিসাব করা হয়েছে।" : "ℹ️ Auto-calculated from item details below."}
                    </p>
                  )}
                </div>
                ...
                <div className={`p-3 rounded-lg border space-y-1.5 text-xs ${
                  isCustom ? "bg-[#18122d] border-[#382b61]" : isDark ? "bg-slate-850 border-slate-700" : "bg-white border-slate-200"
                }`}>
                  {currentCategory && currentFY && (
                    <>
                      <div className="flex justify-between items-center opacity-85">
                        <span>{language === "bn" ? `মূল বাজেট বরাদ্দ:` : `Initial Budget:`}</span>
                        <span className="font-mono font-medium">{formatCurrency(initialBudget)}</span>
                      </div>
                      {provisionAmount > 0 && (
                        <div className="flex justify-between items-center text-amber-500 dark:text-amber-400">
                          <span>{language === "bn" ? "প্রভিশন / সমন্বয়:" : "Provision / Adjustment:"}</span>
                          <span className="font-mono font-medium">+ {formatCurrency(provisionAmount)}</span>
                        </div>
                      )}
                      {additionalBudget > 0 && (
                        <div className="flex justify-between items-center text-blue-500 dark:text-blue-400">
                          <span>{language === "bn" ? "অতিরিক্ত বরাদ্দ (মোট):" : "Additional Allocation:"}</span>
                          <span className="font-mono font-medium">+ {formatCurrency(additionalBudget)}</span>
                        </div>
                      )}
                      {(provisionAmount > 0 || additionalBudget > 0) && (
                        <div className="flex justify-between items-center font-semibold pt-1 border-t border-slate-200 dark:border-slate-700">
                          <span>{language === "bn" ? "সর্বমোট বাজেট:" : "Total Budget:"}</span>
                          <span className="font-mono font-bold">{formatCurrency(totalAllocated)}</span>
                        </div>
                      )}
                      <div className="flex justify-between items-center opacity-80 pt-1">
                        <span>{language === "bn" ? "অনুমোদিত খরচ:" : "Approved Spent:"}</span>
                        <span className="font-mono font-medium text-emerald-500 dark:text-emerald-400">{formatCurrency(totalSpentApproved)}</span>
                      </div>
                      {totalPending > 0 && (
                        <div className="flex justify-between items-center opacity-80">
                          <span>{language === "bn" ? "পেন্ডিং অনিক্রেতা ব্যয়:" : "Pending Expenses:"}</span>
                          <span className="font-mono font-medium text-amber-500 dark:text-amber-400">{formatCurrency(totalPending)}</span>
                        </div>
                      )}
                      <div className={`border-t my-1.5 ${isCustom ? "border-[#2e234e]" : isDark ? "border-slate-750" : "border-slate-100"}`} />
                    </>
                  )}
                  <div className="flex justify-between items-center font-medium">
                    <span className={isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}>{t.availableForUse}:</span>
                    <span className={`font-bold font-mono text-sm ${availableBalance < 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
              ...
              {expenseType === "Quotation" && quotationFormType === "Form1" && (
                <div className="space-y-4">
                  ...
                  <div className={`p-4 rounded-xl border space-y-3 ${
                    isCustom ? "bg-[#140e29] border-[#382b61]" : isDark ? "bg-slate-800/70 border-slate-700" : "bg-white border-slate-300 shadow-sm"
                  }`}>
                    <div className={`flex items-center gap-2 font-bold px-3 py-1.5 rounded-lg text-sm text-white ${
                      isCustom ? "bg-[#32235c]" : "bg-emerald-700"
                    }`}>
                      <span>{language === "bn" ? "সাপ্লাই অর্ডার প্রাপকের তথ্য" : "Supply Order Recipient Info"}</span>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
                      <div>
                        <label className={`block font-semibold text-xs mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-800"}`}>
                          {language === "bn" ? "জনাবের নাম" : "Recipient Name"}
                        </label>
                        <input
                          type="text"
                          value={supplyRecipientName}
                          onChange={e => setSupplyRecipientName(e.target.value)}
                          placeholder="যেমন: মোহাম্মদ আলী"
                          className={`w-full px-3 py-2 border rounded-xl focus:outline-none text-xs ${
                            isCustom ? "bg-[#18122d] border-[#382b61] text-purple-100" : isDark ? "bg-slate-850 border-slate-700 text-slate-100" : "bg-white border-slate-300 text-slate-900 focus:border-emerald-600"
                          }`}
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          onChange={e => setSupplyRecipientDesignation(e.target.value)}
                      ...
                      <div className="flex justify-between items-center mb-1">
                        ...
                        {form1LowestBidder.name && (
                          <span className="text-[10px] text-emerald-700 dark:text-emerald-400 font-semibold">
                            {language === "bn" ? "← সর্বনিম্ন দর প্রদানকারী থেকে অটো" : "← Auto from Lowest Bidder"}
                          </span>
                        )}
                      </div>
                      <input
                        ...
                        value={supplyRecipientOrgName || form1LowestBidder.name || ""}
                        onChange={e => setSupplyRecipientOrgName(e.target.value)}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        <label className={`block font-semibold text-xs mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-800"}`}>
                          {language === "bn" ? "ঠিকানা লাইন ১ (রাস্তা/এলাকা)" : "Address Line 1"}
                        </label>
                        <input
                          type="text"
                          value={supplyRecipientAddress1 || form1LowestBidder.parsed.address1 || ""}
                          onChange={e => setSupplyRecipientAddress1(e.target.value)}
                      ...
                      <div>
                        <label className={`block font-semibold text-xs mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-800"}`}>
                          {language === "bn" ? "ঠিকানা লাইন ২ (থানা/শহর)" : "Address Line 2"}
                        </label>
                        <input
                          type="text"
                          value={supplyRecipientAddress2 || form1LowestBidder.parsed.address2 || ""}
                          onChange={e => setSupplyRecipientAddress2(e.target.value)}
                  ...
                  <div className={`p-4 rounded-xl border space-y-4 ${
                    isCustom ? "bg-[#140e29] border-[#382b61]" : isDark ? "bg-slate-800/70 border-slate-700" : "bg-slate-50 border-slate-300"
                  }`}>
                  <div className="flex justify-between items-center">
                    <div className={`flex items-center gap-1.5 font-bold ${
                      isCustom ? "text-amber-300" : isDark ? "text-slate-200" : "text-slate-900"
                    }`}>
                      <Receipt className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> {language === "bn" ? "পণ্য/সেবা বিবরণ (Quotation Items)" : "Quotation Items"}
                    </div>
                    <button
                      type="button"
                      onClick={() => {
                        const previousItem = quotationItems[quotationItems.length - 1] || quotationItems[0];
                        const inheritedSuppliers = (previousItem && previousItem.suppliers && previousItem.suppliers.length > 0)
                          ? previousItem.suppliers.map((s, sIdx) => ({
                              ...
                              vatRate: 0,
                              taxRate: 0,
                              remarks: s.remarks || (sIdx === 0 ? "সর্বনিম্ন" : sIdx === 1 ? "২য় সর্বোচ্চ" : "সর্বোচ্চ")
                            }))
                          : [
                              { nameAndAddress: "", unitPrice: 0, qty: 1, totalPrice: 0, vatRate: 0, taxRate: 0, remarks: "" }
                            ];
                    ...
                    >
                      <Plus className="w-3.5 h-3.5" /> {language === "bn" ? "＋ নতুন আইটেম যোগ করুন" : "+ Add New Item"}
                    </button>
                  </div>

                  <div className="space-y-4">
                    {quotationItems.map((item, index) => (
                      <div key={index} className={`p-4 rounded-xl border space-y-3 relative ${
                        isCustom ? "bg-[#1c1535] border-[#40316b]" : isDark ? "bg-slate-850 border-slate-700" : "bg-white border-slate-300 shadow-sm"
                      }`}>
                        <div className="flex justify-between items-center border-b pb-2">
                          <span className="font-bold text-emerald-700 dark:text-emerald-400 flex items-center gap-1.5">
                            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs ${
                              isDark ? "bg-emerald-500/25 text-emerald-300" : "bg-emerald-100 text-emerald-800 font-bold"
                            }`}>{index + 1}</span>
                            {language === "bn" ? `আইটেম ${index + 1}` : `Item ${index + 1}`}
                          </span>
                          {quotationItems.length > 1 && (
                            <button
                              ...
                              title={language === "bn" ? "আইটেম মুছে ফেলুন" : "Remove Item"}
                            >
                              <Trash2 className="w-4 h-4" />
                            </button>
                          )}
                        ...
                        <div>
                          <label className={`block font-semibold mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-800"}`}>
                            {language === "bn" ? "আইটেম/পণ্যের বিবরণ *" : "Item Description *"}
                          </label>
                          <input
                            type="text"
                            required
                            ...
                            className={`w-full px-3 py-2 border rounded-xl focus:outline-none ${
                              isCustom ? "bg-[#18122d] border-[#382b61] text-purple-100" : isDark ? "bg-slate-800 border-slate-700 text-slate-100" : "bg-white border-slate-300 text-slate-900"
                            }`}
                          />
                        </div>
                        ...
                        <div>
                          <label className={`block font-semibold mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-800"}`}>
                            {language === "bn" ? "বিস্তারিত বিবরণ / Specification" : "Specification"}
                          </label>
                          <textarea
                            rows={2}
                        ...
                        <div className="grid grid-cols-4 gap-2">
                          <div>
                            ...
                            <input
                              ...
                              onChange={e => {
                                ...
                                updated[index].totalPrice = qty * (updated[index].unitPrice || 0);
                                if (updated[index].suppliers) {
                                  updated[index].suppliers = updated[index].suppliers!.map(s => ({
                                    ...s,
                                    qty: qty,
                          ...
                          <div>
                           ...
                           <div>
                            <label className={`block font-semibold mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-800"}`}>
                              {language === "bn" ? "একক মূল্য (সর্বনিম্ন অটো)" : "Unit Price (Auto Min)"}
                            </label>
                            <input
                              type="number"
                              readOnly
                              ...
                              className={`w-full px-3 py-2 border rounded-xl focus:outline-none font-mono font-bold ${
                                isDark ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/30" : "bg-emerald-50 text-emerald-800 border-emerald-300"
                              }`}
                              title={language === "bn" ? "সর্বনিম্ন দরদাতা থেকে স্বয়ংক্রিয়ভাবে বসবে" : "Auto-populated from lowest bidder"}
                            />
                          </div>
                          <div>
                            <label className={`block font-semibold mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-800"}`}>
                              {language === "bn" ? "মোট মূল্য (অটো)" : "Total Price"}
                            </label>
                            <input
                              type="number"
                              readOnly
                        ...
                        <div className={`p-3 rounded-lg border space-y-2 mt-2 ${
                          isCustom ? "bg-[#120d24] border-[#382b61]" : isDark ? "bg-slate-800 border-slate-700" : "bg-slate-100 border-slate-300"
                        }`}>
                          <div className="flex justify-between items-center">
                            <span className="font-semibold text-emerald-700 dark:text-emerald-400">
                              {language === "bn" ? "সরবরাহকারী প্রতিষ্ঠানের দরপত্রসমূহ (Suppliers)" : "Suppliers Quotations"}
                            </span>
                            <button
                              type="button"
                              onClick={() => {
                                const updated = [...quotationItems];
                                if (!updated[index].suppliers) updated[index].suppliers = [];
                                const sLen = updated[index].suppliers.length;
                                // Suggest name from another item at this supplier index if available
                                const suggestedName = quotationItems.find((_, itmIdx) => itmIdx !== index)?.suppliers?.[sLen]?.nameAndAddress || "";
                                updated[index].suppliers.push({
                                  ...
                                  vatRate: 0,
                                  taxRate: 0,
                                  remarks: sLen === 0 ? "সর্বনিম্ন" : "সর্বোচ্চ"
                                });
                                setQuotationItems(updated);
                              }}
                              className={`text-xs px-2 py-1 rounded-lg font-semibold transition flex items-center gap-1 ${
                                isDark ? "bg-emerald-600/20 text-emerald-400 hover:bg-emerald-600/30" : "bg-emerald-100 text-emerald-800 hover:bg-emerald-200 border border-emerald-300"
                              }`}
                            >
                              <Plus className="w-3 h-3" /> {language === "bn" ? "সরবরাহকারী যোগ করুন" : "Add Supplier"}
                            </button>
                          </div>

                          <div className={`grid grid-cols-12 gap-2 text-[11px] font-semibold px-1 ${
                            isDark ? "opacity-70 text-slate-300" : "text-slate-800"
                          }`}>
                            <div className="col-span-4">{language === "bn" ? "প্রতিষ্ঠানের নাম ও ঠিকানা" : "Name & Address"}</div>
                            <div className="col-span-2">{language === "bn" ? "একক মূল্য" : "Unit Price"}</div>
                            <div className="col-span-2">{language === "bn" ? "মোট মূল্য (অটো)" : "Total Price"}</div>
                            <div className="col-span-3">{language === "bn" ? "মন্তব্য (অটো)" : "Remarks"}</div>
                            <div className="col-span-1 text-center">#</div>
                          </div>

                          {item.suppliers && item.suppliers.map((sup, sIndex) => (
                            <div key={sIndex} className={`grid grid-cols-12 gap-2 items-center pt-2 border-t ${
                              isDark ? "border-slate-500/20" : "border-slate-300"
                            }`}>
                              <div className="col-span-4">
                                <input
                                  type="text"
                                  list="supplier-suggestions"
                                  placeholder={language === "bn" ? `সরবরাহকারী ${sIndex + 1} নাম ও ঠিকানা` : `Supplier ${sIndex + 1} Name & Address`}
                                  value={sup.nameAndAddress}
                                  onChange={e => {
                                    const updated = [...quotationItems];
                                    updated[index].suppliers![sIndex].nameAndAddress = e.target.value;
                                    setQuotationItems(updated);
                                  }}
                              ...
                              <div className="col-span-2">
                                <input
                                  type="number"
                                  placeholder={language === "bn" ? "দর" : "Unit Price"}
                                  value={sup.unitPrice}
                                  onChange={e => {
                                    const updated = [...quotationItems];
                                    const price = Number(e.target.value) || 0;
                                    const suppliers = updated[index].suppliers!;
                                    suppliers[sIndex].unitPrice = price;
                                    suppliers[sIndex].totalPrice = item.qty * price;
                                    ...
                                    updated[index].totalPrice = item.qty * minPrice;
                                    setQuotationItems(updated);
                                  }}
                              ...
                              <div className="col-span-1 text-center">
                                {item.suppliers!.length > 1 && (
                                  <button
                                    ...
                                    onClick={() => {
                                      const updated = [...quotationItems];
                                      const suppliers = updated[index].suppliers!.filter((_, i) => i !== sIndex);
                  ...
                  <div className={`rounded-xl border overflow-hidden shadow-sm ${
                    isCustom ? "bg-[#18122d] border-[#382b61]" : isDark ? "bg-slate-900 border-slate-700" : "bg-white border-emerald-300"
                  }`}>
                    {/* Header Bar */}
                    <div className="bg-[#15803d] text-white px-4 py-2.5 flex items-center justify-between flex-wrap gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-sm">১১১৪ শাখা এন্ট্রি (ডেবিট সারি)</span>
                        <span className="text-[11px] text-emerald-100 hidden md:inline font-normal">
                          (পূরণ করা হলে ফরওয়ার্ডিং এ 1114 শাখাভিত্তিক ডেবিট ও ১৪৮/০১ ক্রেডিট সমন্বয় প্রদর্শিত হবে)
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        {quotationItems.length > 0 && (
                          <button
                            type="button"
                            onClick={() => {
                              const branchOfficeList = offices.filter(o => o.id !== officeId && o.type !== "HeadOffice");
                              const effectiveBranches = branchOfficeList.length > 0 ? branchOfficeList : offices;
                              const autoEntries: BranchDebitEntry[] = quotationItems.map((qItem, idx) => {
                                const targetBranch = effectiveBranches[idx % effectiveBranches.length] || offices[0];
                                const branchCleanName = targetBranch.name.includes("শাখা") || targetBranch.name.includes("অফিস") 
                                  ? targetBranch.name 
                                  : `${targetBranch.name} শাখা`;
                                const branchLabel = `${branchCleanName} (${targetBranch.code || ''})`.trim();
                                const uPrice = Number(qItem.unitPrice || 0);
                                const q = Number(qItem.qty || 1);
                                const totalP = Number(qItem.totalPrice) || (uPrice * q);
                                return {
                                  branchOfficeId: targetBranch.id,
                                  branchName: branchLabel,
                                  itemIndex: idx,
                                  itemDescription: qItem.itemDescription || "",
                                  model: qItem.model || qItem.specification || "",
                                  unit: qItem.unit || "টি",
                                  qty: q,
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            const unitPrice = firstItem ? Number(firstItem.unitPrice || 0) : 0;
                            const branchOfficeList = offices.filter(o => o.id !== officeId);
                            const defaultBranch = branchOfficeList[0] || offices[0];
                            const branchCleanName = defaultBranch?.name.includes("শাখা") || defaultBranch?.name.includes("অফিস")
                              ? defaultBranch.name
                              : `${defaultBranch?.name || 'শাখা'} শাখা`;
                            const newEntry: BranchDebitEntry = {
                              branchOfficeId: defaultBranch?.id || "",
                              branchName: defaultBranch ? `${branchCleanName} (${defaultBranch.code || ''})`.trim() : "শাখা",
                              itemIndex: 0,
                              itemDescription: firstItem?.itemDescription || "",
                              model: firstItem?.model || firstItem?.specification || "",
                              unit: firstItem?.unit || "টি",
                              qty: 1,
                              amount: Math.round(unitPrice * 1 * (1 + (effectiveVatRate + effectiveTaxRate) / 100))
                            };
                      ...
                      {branchEntries.length === 0 ? (
                        ...
                        <>
                          ...
                          <div className="grid grid-cols-12 gap-2 text-xs font-bold text-slate-700 dark:text-slate-300 pb-1 border-b dark:border-slate-700">
                            <div className="col-span-12 sm:col-span-3">শাখার নাম</div>
                            <div className="col-span-12 sm:col-span-4">পণ্য আইটেম</div>
                            <div className="col-span-5 sm:col-span-2 text-center">পরিমাণ (Qty)</div>
                            <div className="col-span-5 sm:col-span-3 text-right">
                              টাকার পরিমাণ (৳)
                            </div>
                            <div className="col-span-2 sm:col-span-0 text-center sm:hidden">অ্যাকশন</div>
                          </div>
                                ...
                                >
                                  ...
                                  <div className="col-span-12 sm:col-span-3">
                                    <select
                                      ...
                                      onChange={(e) => {
                                        ...
                                        const matched = offices.find(o => `${o.name} (${o.code})` === val || o.name === val || `${o.name} শাখা (${o.code})` === val);
                                        setBranchEntries(prev => {
                                          const copy = [...prev];
                                          copy[bIdx] = {
                                            ...copy[bIdx],
                                            branchName: val,
                                            branchOfficeId: matched?.id || copy[bIdx].branchOfficeId
                                          };
                                          return copy;
                                        });
                                      }}
                                    ...
                                    >
                                      {offices.map(o => {
                                        const cleanName = o.name.includes("শাখা") || o.name.includes("অফিস") || o.name.includes("কার্যালয়")
                                          ? o.name
                                          : `${o.name} শাখা`;
                                        const label = `${cleanName} (${o.code || ''})`.trim();
                                        return (
                                          <option key={o.id} value={label}>
                                            {cleanName} {o.code ? `(${o.code})` : ''}
                                          </option>
                                        );
                                      })}
                                    </select>
                                  ...
                                  <div className="col-span-12 sm:col-span-4">
                                    <select
                                      ...
                                      onChange={(e) => {
                                        const itemIdx = Number(e.target.value);
                                        const selItem = quotationItems[itemIdx];
                                        const uPrice = selItem ? Number(selItem.unitPrice || 0) : 0;
                                        setBranchEntries(prev => {
                                          const copy = [...prev];
                                          copy[bIdx] = {
                                            ...copy[bIdx],
                                            itemIndex: itemIdx,
                                            itemDescription: selItem?.itemDescription || "",
                                            model: selItem?.model || selItem?.specification || "",
                                            unit: selItem?.unit || "টি",
                                            amount: Math.round(uPrice * (copy[bIdx].qty || 1) * (1 + (effectiveVatRate + effectiveTaxRate) / 100))
                                          };
                                          return copy;
                                        });
                                      }}
                                    ...
                                    >
                                      {quotationItems.map((qi, qIdx) => (
                                        <option key={qIdx} value={qIdx}>
                                          {qIdx + 1}. {qi.itemDescription || `আইটেম ${qIdx + 1}`} {qi.specification ? `(${qi.specification})` : ''}
                                        </option>
                                      ))}
                                    </select>
                                  </div>
                                  ...
                                  <div className="col-span-5 sm:col-span-2">
                                    <input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        const selItem = quotationItems[branch.itemIndex ?? 0];
                                        const uPrice = selItem ? Number(selItem.unitPrice || 0) : 0;
                                        setBranchEntries(prev => {
                                          const copy = [...prev];
                                          copy[bIdx] = {
                                            ...copy[bIdx],
                                            qty: newQty,
                                            amount: Math.round(uPrice * newQty * (1 + (effectiveVatRate + effectiveTaxRate) / 100))
                                          };
                                          return copy;
                                        });
                                      }}
                                  ...
                                  <div className="col-span-5 sm:col-span-3 flex items-center gap-1.5">
                                    <input
                                      ...
                                      value={branch.amount}
                                      onChange={(e) => {
                                        const customAmt = Number(e.target.value) || 0;
                                        setBranchEntries(prev => {
                                          const copy = [...prev];
                                          copy[bIdx] = {
                                            ...copy[bIdx],
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => {
                                        setBranchEntries(prev => prev.filter((_, idx) => idx !== bIdx));
                                      }}
                          ...
                          <div className="flex justify-end items-center pt-2 border-t dark:border-slate-700">
                            <div className="text-xs font-bold text-slate-800 dark:text-emerald-300 flex items-center gap-2">
                              ...
                              <span className="font-mono text-sm text-emerald-700 dark:text-emerald-400">
                                ৳ {branchEntries.reduce((acc, b) => acc + (Number(b.amount) || 0), 0).toLocaleString('en-IN')}
                              </span>
                            </div>
                          </div>
                        </>
                  ...
                  <div className={`p-3.5 rounded-xl border space-y-2 text-xs font-mono ${
                    isCustom ? "bg-[#110c22] border-[#382b61] text-purple-100" : isDark ? "bg-slate-900 border-slate-700" : "bg-slate-100 border-slate-300 text-slate-900"
                  }`}>
                    <div className={`flex justify-between items-center font-sans font-bold pb-1 border-b ${
                      isCustom ? "text-amber-300 border-[#382b61]" : isDark ? "text-emerald-400 border-emerald-500/20" : "text-emerald-800 border-emerald-300"
                    }`}>
                      <span>{language === "bn" ? "হিসাবের সারাংশ (Summary)" : "Calculation Summary"}</span>
                      <span>{language === "bn" ? "স্বয়ংক্রিয় হিসাব" : "Auto Calculated"}</span>
                    </div>
                    <div className={`flex justify-between items-center ${isCustom ? "text-purple-200" : isDark ? "opacity-80" : "text-slate-800 font-medium"}`}>
                      <span>{language === "bn" ? "মোট আইটেম মূল্য (Base Total):" : "Total Item Value:"}</span>
                      <span>{formatCurrency(quotationItems.reduce((acc, item) => acc + (item.totalPrice || 0), 0))}</span>
                    </div>
                    <div className={`flex justify-between items-center ${isCustom ? "text-purple-200" : isDark ? "opacity-80" : "text-slate-800 font-medium"}`}>
                      <span>{language === "bn" ? `ভ্যাট (${effectiveVatRate}%):` : `VAT Amount (${effectiveVatRate}%):`}</span>
                      <span>{formatCurrency((quotationItems.reduce((acc, item) => acc + (item.totalPrice || 0), 0) * effectiveVatRate) / 100)}</span>
                    </div>
                    <div className={`flex justify-between items-center ${isCustom ? "text-purple-200" : isDark ? "opacity-80" : "text-slate-800 font-medium"}`}>
                      <span>
                        {language === "bn" ? `ট্যাক্স (${effectiveTaxRate}%):` : `Tax Amount (${effectiveTaxRate}%):`}
                        {hasStockChalan === "হ্যাঁ" && (
                          <span className="text-[10px] text-emerald-600 dark:text-emerald-400 ml-1 font-sans font-bold">(মুসক চালানে ০%)</span>
                        )}
                      </span>
                      <span>{formatCurrency((quotationItems.reduce((acc, item) => acc + (item.totalPrice || 0), 0) * effectiveTaxRate) / 100)}</span>
                    </div>
                    <div className={`border-t pt-1.5 flex justify-between items-center font-bold text-sm ${
                      isCustom ? "text-amber-300 border-[#382b61]" : isDark ? "text-emerald-500 border-slate-700" : "text-emerald-800 border-emerald-300"
                    }`}>
                      <span>{language === "bn" ? "সর্বমোট প্রদেয় (Grand Total):" : "Grand Total:"}</span>
                      <span className="font-mono">
                        {formatCurrency(
                          quotationItems.reduce((acc, item) => acc + (item.totalPrice || 0), 0) +
                          (quotationItems.reduce((acc, item) => acc + (item.totalPrice || 0), 0) * (effectiveVatRate + effectiveTaxRate)) / 100
                        )}
                      </span>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className={`p-4 rounded-xl border space-y-3 ${
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className="text-[10px] text-emerald-700 dark:text-emerald-400 font-semibold">
                          {language === "bn" ? `← সর্বনিম্ন দর প্রদানকারী (প্রতিষ্ঠান ${form2LowestBidder.index}) থেকে অটো` : `← Auto from Lowest Bidder (Org ${form2LowestBidder.index})`}
                        </span>
                      </div>
                      <input
                        type="text"
                        list="supplier-suggestions"
                        value={supplyRecipientOrgName || form2LowestBidder.name || supplierOrg1}
                        onChange={e => setSupplyRecipientOrgName(e.target.value)}
                        placeholder="যেমন: ইউনিক প্রিন্টার্স, ১৬ আলিফ বিপণি সেন্টার..."
                        className={`w-full px-3 py-2 border rounded-xl focus:outline-none text-xs ${
                          isCustom ? "bg-[#18122d] border-[#382b61] text-purple-100" : isDark ? "bg-slate-850 border-slate-700 text-slate-100" : "bg-white border-slate-300 text-slate-900 focus:border-emerald-600"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          type="text"
                          value={supplyRecipientAddress1 || form2LowestBidder.parsed.address1 || "নজির আহমেদ চৌধুরী রোড"}
                          onChange={e => setSupplyRecipientAddress1(e.target.value)}
                          placeholder="যেমন: নজির আহমেদ চৌধুরী রোড"
                          className={`w-full px-3 py-2 border rounded-xl focus:outline-none text-xs ${
                            isCustom ? "bg-[#18122d] border-[#382b61] text-purple-100" : isDark ? "bg-slate-850 border-slate-700 text-slate-100" : "bg-white border-slate-300 text-slate-900 focus:border-emerald-600"
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={supplyRecipientAddress2 || form2LowestBidder.parsed.address2 || "আন্দরকিল্লা, চট্টগ্রাম।"}
                          onChange={e => setSupplyRecipientAddress2(e.target.value)}
                          placeholder="যেমন: আন্দরকিল্লা, চট্টগ্রাম।"
                          className={`w-full px-3 py-2 border rounded-xl focus:outline-none text-xs ${
                            isCustom ? "bg-[#18122d] border-[#382b61] text-purple-100" : isDark ? "bg-slate-850 border-slate-700 text-slate-100" : "bg-white border-slate-300 text-slate-900 focus:border-emerald-600"
                  ...
                  <div className={`p-4 rounded-xl border space-y-3 ${
                    ...
                    <div className={`flex items-center gap-2 font-bold px-3 py-1.5 rounded-lg text-sm text-white ${
                      ...
                      <span>{language === "bn" ? "দরপত্রদাতা প্রতিষ্ঠানসমূহের নাম ও ঠিকানা" : "Quotation Bidder Organizations"}</span>
                    </div>

                    <div className="space-y-3 pt-1">
                      <div>
                        <label className={`block font-semibold text-xs mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-800"}`}>
                          {language === "bn" ? "প্রতিষ্ঠান ১ *" : "Organization 1 *"}
                        </label>
                        <input
                          type="text"
                          required
                          list="supplier-suggestions"
                          value={supplierOrg1}
                          onChange={e => handleForm2SupplierNameChange(1, e.target.value)}
                      ...
                      <div>
                        <label className={`block font-semibold text-xs mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-800"}`}>
                          {language === "bn" ? "প্রতিষ্ঠান ২ *" : "Organization 2 *"}
                        </label>
                        <input
                          type="text"
                          required
                          list="supplier-suggestions"
                          value={supplierOrg2}
                          onChange={e => handleForm2SupplierNameChange(2, e.target.value)}
                      ...
                      <div>
                        <label className={`block font-semibold text-xs mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-800"}`}>
                          {language === "bn" ? "প্রতিষ্ঠান ৩ *" : "Organization 3 *"}
                        </label>
                        <input
                          type="text"
                          required
                          list="supplier-suggestions"
                          value={supplierOrg3}
                          onChange={e => handleForm2SupplierNameChange(3, e.target.value)}
                  ...
                  <div className={`p-4 rounded-xl border space-y-4 ${
                    ...
                    <div className="flex justify-between items-center">
                      <div className={`flex items-center gap-2 font-bold px-3 py-1.5 rounded-lg text-sm text-white ${
                        ...
                        <span>{language === "bn" ? "পণ্য আইটেম সমূহ" : "Quotation Items"}</span>
                      </div>
                      <button
                        type="button"
                    ...
                    <div className="space-y-4">
                      {form2Items.map((item, itemIdx) => (
                        <div key={itemIdx} className={`p-4 rounded-xl border space-y-3 ${
                          isCustom ? "bg-[#1c1535] border-[#40316b]" : isDark ? "bg-slate-850 border-slate-700" : "bg-slate-50 border-slate-300"
                        }`}>
                          <div className="flex justify-between items-center border-b pb-2">
                            <span className="font-bold text-emerald-800 dark:text-emerald-400 flex items-center gap-2 text-sm">
                              <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs ${
                                isDark ? "bg-emerald-500/25 text-emerald-300" : "bg-emerald-200 text-emerald-900 font-bold"
                              }`}>{itemIdx + 1}</span>
                              {language === "bn" ? `আইটেম ${itemIdx + 1}` : `Item ${itemIdx + 1}`}
                            </span>
                            {form2Items.length > 1 && (
                              <button
                                type="button"
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
                            <div className="md:col-span-6">
                              <label className={`block font-semibold text-xs mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-800"}`}>
                                {language === "bn" ? "দ্রব্যাদির নাম ও বিবরণ *" : "Item Description *"}
                              </label>
                              <input
                                type="text"
                                required
                                value={item.itemDescription}
                                onChange={e => handleForm2ItemDescChange(itemIdx, e.target.value)}
                                placeholder="যেমন: সঞ্চয়ী হিসাব জমা বই"
                                className={`w-full px-3 py-2 border rounded-xl focus:outline-none text-xs ${
                                  isCustom ? "bg-[#18122d] border-[#382b61] text-purple-100" : isDark ? "bg-slate-800 border-slate-700 text-slate-100" : "bg-white border-slate-300 text-slate-900 focus:border-emerald-600"
                                }`}
                              />
                            </div>
                            <div className="md:col-span-3">
                              <label className={`block font-semibold text-xs mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-800"}`}>
                                {language === "bn" ? "পরিমাণ (সংখ্যায়) *" : "Quantity (Number) *"}
                              </label>
                              <input
                                type="number"
                                required
                                min="1"
                                value={item.qty}
                                onChange={e => handleForm2QtyChange(itemIdx, Number(e.target.value) || 1)}
                            ...
                            <div className="md:col-span-3">
                              ...
                              <select
                                value={item.unit}
                                onChange={e => handleForm2UnitChange(itemIdx, e.target.value)}
                                className={`w-full px-3 py-2 border rounded-xl focus:outline-none text-xs font-medium ${
                                  isCustom ? "bg-[#18122d] border-[#382b61] text-purple-100" : isDark ? "bg-slate-800 border-slate-700 text-slate-100" : "bg-white border-slate-300 text-slate-900 focus:border-emerald-600"
                                }`}
                              >
                          ...
                          <div className="overflow-x-auto rounded-xl border border-slate-300 dark:border-slate-700 mt-2">
                            <table className="w-full text-xs text-left">
                              <thead className="bg-emerald-700 text-white font-semibold">
                                <tr>
                                  <th className="p-2.5">{language === "bn" ? "প্রতিষ্ঠান (অটো: উপরের নাম)" : "Organization"}</th>
                                  <th className="p-2.5 w-28 text-center">{language === "bn" ? "একক দর (৳)" : "Unit Price (৳)"}</th>
                                  <th className="p-2.5 w-20 text-center">{language === "bn" ? "পরিমাণ" : "Qty"}</th>
                                  <th className="p-2.5 w-28 text-right">{language === "bn" ? "মোট মূল্য (৳) ←অটো" : "Total (৳)"}</th>
                                  <th className="p-2.5 w-32 text-center">{language === "bn" ? "মন্তব্য" : "Remarks"}</th>
                                </tr>
                              </thead>
                              <tbody className="divide-y divide-slate-200 dark:divide-slate-700/60 bg-white dark:bg-slate-900">
                                {item.suppliers.map((sup, sIdx) => {
                                  const currentOrgName = sIdx === 0 ? supplierOrg1 : sIdx === 1 ? supplierOrg2 : supplierOrg3;
                                  return (
                                    <tr key={sIdx} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                                      <td className="p-2.5">
                                        <div className="font-semibold text-slate-900 dark:text-slate-100 flex items-center gap-1.5">
                                          <span className="text-emerald-700 dark:text-emerald-400 font-bold">
                                            {sIdx === 0 ? "🏢 প্রতিষ্ঠান ১:" : sIdx === 1 ? "🏢 প্রতিষ্ঠান ২:" : "🏢 প্রতিষ্ঠান ৩:"}
                                          </span>
                                          <span className="truncate max-w-[280px] font-normal text-slate-700 dark:text-slate-300" title={currentOrgName}>
                                            {currentOrgName ? currentOrgName.split(",")[0] : `প্রতিষ্ঠান ${sIdx + 1}`}
                                          </span>
                                        </div>
                                      </td>
                                      <td className="p-2 text-center">
                                        <input
                                          ...
                                          min="0"
                                          value={sup.unitPrice}
                                          onChange={e => handleForm2PriceChange(itemIdx, sIdx, Number(e.target.value) || 0)}
                                      ...
                                      <td className="p-2.5 text-right font-mono font-bold text-slate-900 dark:text-slate-100">
                                        ৳{((sup.unitPrice || 0) * item.qty).toFixed(2)}
                                      </td>
                    ...
                    <div className="p-3.5 rounded-xl border-2 border-emerald-500 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-950 dark:text-emerald-100 text-xs md:text-sm font-bold flex flex-wrap items-center justify-between gap-2 shadow-sm">
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
                        <span>
                          {language === "bn" ? "সর্বনিম্ন দরদাতার মোট মূল্য" : "Lowest Bidder Base Total"}: <span className="font-mono text-emerald-700 dark:text-emerald-300">৳ {form2LowestBaseTotal.toFixed(2)}</span>
                        </span>
                        <span className="text-slate-400">|</span>
                        <span>
                          {effectiveVatRate}% {language === "bn" ? "ভ্যাট" : "VAT"}: <span className="font-mono text-emerald-700 dark:text-emerald-300">৳ {form2VatAmount.toFixed(2)}</span>
                        </span>
                        <span className="text-slate-400">|</span>
                        <span>
                          {effectiveTaxRate}% {language === "bn" ? "ট্যাক্স" : "TAX"}{hasStockChalan === "হ্যাঁ" ? " (মুসক চালানে ০%)" : ""}: <span className="font-mono text-emerald-700 dark:text-emerald-300">৳ {form2TaxAmount.toFixed(2)}</span>
                        </span>
                      </div>
                      <div className="border-t md:border-t-0 md:border-l border-emerald-300 dark:border-emerald-700 pt-1 md:pt-0 md:pl-4">
                        <span className="text-emerald-900 dark:text-emerald-200">
                          {language === "bn" ? "কর্তনযোগ্য সর্বমোট" : "Grand Total"}: <span className="font-mono text-base text-emerald-800 dark:text-emerald-300 font-extrabold">৳ {form2GrandTotal.toFixed(2)}</span>
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              {expenseType !== "Quotation" && (
                <div className={`p-4 rounded-xl border space-y-3 ${
                  ...
                  <div className={`flex items-center gap-1.5 font-bold ${
                    ...
                    <UserCircle className="w-4 h-4 text-emerald-500" /> {t.applicantType}
                  </div>
                  
                  <div className="flex gap-4">
                    <label className={`flex items-center gap-2 cursor-pointer font-medium text-xs md:text-sm ${
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      <label className={`block font-semibold text-xs mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>{t.applicantName}</label>
                      <input
                        type="text"
                        value={applicantName}
                        ...
                        className={`w-full px-3 py-2 border rounded-xl focus:outline-none text-xs ${
                          isCustom ? "bg-[#18122d] border-[#382b61] text-purple-100 placeholder:text-purple-400/50" : isDark ? "bg-slate-850 border-slate-700 text-slate-100 placeholder:text-slate-500" : "bg-white border-slate-300 text-slate-900 focus:border-emerald-600"
                        }`}
                      />
                    </div>
                    <div>
                      <label className={`block font-semibold text-xs mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>{t.applicantDesignation}</label>
                      <input
                        type="text"
                  ...
                  {applicantType === "PersonInstitution" && (
                    <div>
                      <label className={`block font-semibold text-xs mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>{t.applicantInstitution}</label>
                      <input
                        type="text"
              ...
              {expenseType !== "Quotation" && (
                <div className="space-y-3">
                  <div>
                    <label className={`block font-semibold mb-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>{t.description} *</label>
                    <textarea
                      required
                      rows={2}
                      value={description}
                      ...
                      className={`w-full px-3 py-2 border rounded-xl focus:outline-none ${
                        isCustom ? "bg-[#140e29] border-[#382b61] text-purple-100 placeholder:text-purple-400/50" : isDark ? "bg-slate-850 border-slate-700 text-slate-100 placeholder:text-slate-500" : "bg-white border-slate-300 text-slate-900"
                      }`}
                    />
                  </div>
                </div>
              ...
              <div className={`p-4 rounded-xl border space-y-2 ${
                ...
                <label className={`block font-semibold flex items-center gap-1.5 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"}`}>
                  <Paperclip className="w-4 h-4 text-emerald-400" />
                  {language === "bn" ? "সংযুক্তি / ভাউচারের কপি (সাপোর্টিং ডকুমেন্ট)" : "Supporting Document / Voucher Copy"}
                </label>
                ...
                {supportingDocument && (
                  <div className={`flex items-center justify-between p-2.5 rounded-lg border text-xs ${
                    ...
                    <div className="flex items-center gap-2 truncate">
                      ...
                      <span className="truncate font-mono">{supportingDocument.split("/").pop()}</span>
                    </div>
              ...
              <div className={`flex justify-end gap-2 pt-2 border-t ${
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl font-medium ${
                    isCustom ? "bg-[#251d45] text-purple-200 hover:bg-[#32285e]" : isDark ? "bg-slate-800 text-slate-300 hover:bg-slate-700" : "text-slate-600 hover:bg-slate-100"
                  }`}
                >
                  {t.cancel}
                </button>
      ...
      {rejectingExpenseId && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex overflow-y-auto p-4 sm:p-6">
          <div className={`m-auto rounded-2xl max-w-md w-full p-6 shadow-2xl border ${
            isCustom ? "bg-[#18122d] border-[#382b61] text-purple-100" : isDark ? "bg-slate-900 border-slate-700 text-slate-100" : "bg-white border-slate-200 text-slate-900"
          }`}>
            <h3 className="text-base font-bold mb-2 flex items-center gap-2 text-rose-400">
              <XCircle className="w-5 h-5" />
              {language === "bn" ? "ব্যয় প্রত্যাখ্যানের কারণ" : "Reject Expense Reason"}
            </h3>
            <p className={`text-xs mb-4 ${isCustom ? "text-purple-300" : isDark ? "text-slate-400" : "text-slate-600"}`}>
              {language === "bn"
            ...
            <textarea
              ...
              placeholder={language === "bn" ? "যেমন: ভাউচারের কপি অস্পষ্ট বা অপ্রতুল বরাদ্দ..." : "Reason..."}
      ...
      {previewNoteSheet && (
        <NoteSheetPreviewModal
          ...
          officeName={offices.find(o => o.id === previewNoteSheet.officeId)?.name}
          categoryId={expenses.find(e => e.noteSheetId === previewNoteSheet.id)?.categoryId}
          categoryName={categories.find(c => c.id === expenses.find(e => e.noteSheetId === previewNoteSheet.id)?.categoryId)?.name}
      ...
      {deletingId && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex overflow-y-auto p-4 sm:p-6">
          <div className={`m-auto rounded-2xl max-w-sm w-full p-6 shadow-2xl border ${
            isCustom ? "bg-[#18122d] border-[#382b61] text-purple-100" : isDark ? "bg-slate-900 border-slate-700 text-slate-100" : "bg-white border-slate-200 text-slate-900"
          }`}>
            <h3 className="text-base font-bold mb-2">{language === "bn" ? "নিশ্চিত করুন" : "Confirm Delete"}</h3>
            <p className={`text-sm mb-6 ${isCustom ? "text-purple-300" : isDark ? "text-slate-400" : "text-slate-600"}`}>
              {language === "bn" ? "আপনি কি নিশ্চিত যে এই ব্যয়ের রেকর্ডটি মুছে ফেলতে চান?" : "Are you sure you want to void this expense?"}
            </p>
            <div className="flex justify-end gap-2">
              <button
                type="button"
      ...
      {/* Toast Notification */}
      {toastMsg && (
        <div className={`fixed bottom-6 right-6 px-6 py-3 rounded-xl shadow-2xl z-50 flex items-center gap-3 border ${
          isCustom ? "bg-[#18122d] border-[#4b3b7a] text-purple-100" : isDark ? "bg-slate-800 border-slate-700 text-white" : "bg-slate-900 border-slate-700 text-white"
        }`}>
