    ...
    >
      <div className="w-full max-w-[1600px] 2xl:max-w-[1800px] mx-auto flex flex-col md:flex-row items-center justify-between gap-2 text-xs">
        ...
        <div className="flex items-center gap-2 whitespace-nowrap overflow-hidden text-ellipsis">
          <div className={`flex items-center gap-1.5 font-semibold shrink-0 ${isCustom ? "text-purple-100" : isDark ? "text-slate-100" : "text-slate-900"}`}>
            <ShieldCheck className={`w-3.5 h-3.5 shrink-0 ${isCustom ? "text-amber-400" : isDark ? "text-emerald-400" : "text-emerald-600"}`} />
            <span>
              {language === "bn" ? "অফিস বরাদ্দ ও ব্যয় ব্যবস্থাপনা সিস্টেম" : "Office Allocation & Expense Management System"}
            </span>
          </div>
        ...
        <div className="flex items-center gap-x-2.5 sm:gap-x-3 shrink-0 whitespace-nowrap text-xs">
          <div className="flex items-center gap-1">
            <span className="opacity-70">{language === "bn" ? "ডেভেলপমেন্ট:" : "Engineered by:"}</span>
            <strong className={`font-semibold ${isCustom ? "text-purple-100" : isDark ? "text-slate-100" : "text-slate-900"}`}>
              PJC Automation
            </strong>
          </div>
          ...
          <div className="flex items-center gap-1">
            ...
            <a
              ...
              title={language === "bn" ? "সহায়তার জন্য কল করুন" : "Call for Technical Support"}
            >
              +8801820302571
            </a>
          </div>
          ...
          {onOpenAbout && (
            <button
              ...
              title={language === "bn" ? "সিস্টেম ও ডেভেলপার পরিচিতি" : "System & Developer Details"}
            >
