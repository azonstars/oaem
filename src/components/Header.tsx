...
import { 
  LogOut, Search, Globe, Sun, Moon, Palette, Menu, 
  ChevronDown, Lock, Info, Building2, Calendar, 
...
export function Header({
  ...
  const parentOffice = currentOffice?.parentOfficeId ? offices.find(o => o.id === currentOffice.parentOfficeId) : null;
  ...
  const replacePlaceholders = (template: string): string => {
    ...
    const fyName = currentFY?.name || (financialYears.find(f => f.isActive)?.name || "2026-2027");
  ...
  const rawNotices = (systemSettings?.notices && systemSettings.notices.length > 0)
  ...
  const handlePrevNotice = () => {
    setCurrentNoticeIndex(prev => (prev - 1 + processedNotices.length) % processedNotices.length);
  };
    ...
    <div className="sticky top-0 z-30 shrink-0 w-full">
        ...
        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
            ...
            {systemSettings?.logoUrl ? (
              ...
              >
                <div className="text-center leading-none">
                  <span className="text-xs sm:text-xs font-extrabold tracking-wider block">গণ</span>
                  <span className="text-[7px] sm:text-[8px] font-semibold opacity-90 block">GOB</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-1.5 mt-0.5 min-w-0">
              <div className={`inline-flex items-center gap-1 px-1.5 sm:px-2 py-0.5 rounded-md text-xs sm:text-xs font-semibold border max-w-full truncate ${
                isCustom
                ...
                <span className="truncate max-w-[120px] sm:max-w-[180px] md:max-w-[240px]">
                  {currentOffice?.name || (language === "bn" ? "প্রধান কার্যালয়, ঢাকা" : "Head Office, Dhaka")}
                </span>
              </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 md:gap-2.5 shrink-0 ml-auto flex-nowrap">
          ...
          <div className="relative hidden xl:block">
            ...
            <input
              ...
              placeholder={t.searchPlaceholder || (language === "bn" ? "অনুসন্ধান করুন..." : "Search anything...")}
          ...
          <div className={`flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-1.5 rounded-xl border text-xs font-semibold shadow-2xs shrink-0 ${
            currentFY?.isClosed
            ...
            >
              {financialYears.map((fy) => (
                <option key={fy.id} value={fy.id} className="text-slate-900 bg-white">
                  {fy.name} {fy.isClosed ? `(${language === "bn" ? "🔒 ক্লোজড" : "🔒 Closed"})` : fy.isActive ? `(${language === "bn" ? "চলতি" : "Active"})` : ""}
                </option>
              ))}
            </select>
            {currentFY?.isClosed && (
              <span className="px-1.5 py-0.5 rounded text-xs sm:text-xs font-bold bg-rose-600 text-white flex items-center gap-0.5 sm:gap-1 shrink-0">
                ...
                <span className="hidden sm:inline">{language === "bn" ? "ক্লোজড" : "Closed"}</span>
              </span>
            )}
          </div>
          ...
          >
            ...
            <span className="hidden lg:inline">{language === "bn" ? "English" : "বাংলা"}</span>
          </button>
              ...
              >
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3.5 py-2 text-left hover:bg-emerald-500/10 transition ${
                    theme === "light" ? "font-bold text-emerald-600 bg-emerald-500/10" : ""
                  }`}
                >
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3.5 py-2 text-left hover:bg-emerald-500/10 transition ${
                    theme === "dark" ? "font-bold text-emerald-400 bg-emerald-500/10" : ""
                  }`}
                >
              ...
              >
                ...
                <div className="py-1">
                  ...
                  >
                    ...
                    <span>{language === "bn" ? "পাসওয়ার্ড পরিবর্তন" : "Change Password"}</span>
                  </button>
                  ...
                  >
                    ...
                    <span>{language === "bn" ? "সিস্টেম পরিচিতি ও সংস্করণ" : "About & System Info"}</span>
                  </button>
                </div>
      ...
      >
        ...
        <div className="flex items-center gap-2 sm:gap-2.5 min-w-0 flex-1">
          <div className={`flex items-center gap-1.5 px-2.5 py-0.5 rounded-full font-bold text-xs shrink-0 border ${
            isCustom
            ...
            <span className="tracking-wide">{language === "bn" ? "বিজ্ঞপ্তি" : "Notice"}</span>
          </div>
        ...
        <div className={`flex items-center gap-1.5 sm:gap-2 shrink-0 text-xs ${
          isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-700"
        }`}>
          ...
          <div className="flex items-center gap-0.5">
            <button
              ...
              title={language === "bn" ? "পূর্ববর্তী বিজ্ঞপ্তি" : "Previous notice"}
            >
