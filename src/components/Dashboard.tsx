...
import { Allocation, Expense, Category, Office, FinancialYear, User, NoteSheet, SystemSettings } from "../types";
import { DollarSign, Receipt, FileText, TrendingUp, AlertCircle, ArrowRightLeft, Clock, ShieldAlert, Code, Phone, MapPin } from "lucide-react";
...
interface DashboardProps {
  ...
  onNavigateExpenses?: (filter: "All" | "Pending" | "Approved" | "Rejected") => void;
  onOpenAbout?: () => void;
}
...
export function Dashboard({
  ...
  const filteredAllocations = allocations.filter(a => {
    ...
    const matchOffice = isHeadOffice ? true : a.officeId === currentUser.officeId;
  ...
  const totalSpent = approvedExpenses.reduce((sum, e) => sum + Number(e.amount || 0), 0);
  ...
  const pendingNoteSheetsCount = filteredExpenses.filter(e => !e.noteSheetId).length;
  ...
  const officeSummary = isHeadOffice ? offices.map(off => {
    const allocated = filteredAllocations
  ...
  const getTimeSlotAndGreeting = (): { slot: "morning" | "afternoon" | "evening" | "night"; greeting: string } => {
    const hour = new Date().getHours();
    if (hour >= 5 && hour < 12) {
      return { slot: "morning", greeting: language === "bn" ? "শুভ সকাল" : "Good Morning" };
    } else if (hour >= 12 && hour < 17) {
      return { slot: "afternoon", greeting: language === "bn" ? "শুভ অপরাহ্ন" : "Good Afternoon" };
    } else if (hour >= 17 && hour < 20) {
      return { slot: "evening", greeting: language === "bn" ? "শুভ সন্ধ্যা" : "Good Evening" };
    } else {
      return { slot: "night", greeting: language === "bn" ? "শুভ রাত্রি" : "Good Night" };
    }
  };
  ...
  const replacePlaceholders = (template: string): string => {
    if (!template) return "";
    const systemName = systemSettings?.webAppName || (language === "bn" ? "অফিস বরাদ্দ ও ব্যয় ব্যবস্থাপনা সিস্টেম" : "Office Allocation & Expense Management System");
    const officeName = currentOffice?.name || (language === "bn" ? "প্রধান কার্যালয়" : "Head Office");
    const fyName = currentFYObj?.name || (financialYears.find(f => f.isActive)?.name || "2026-2027");
    const userName = currentUser?.name || (language === "bn" ? "ব্যবহারকারী" : "User");
    const institutionName = systemSettings?.institutionName || (language === "bn" ? "গণপ্রজাতন্ত্রী বাংলাদেশ সরকার" : "Government of the People's Republic of Bangladesh");
  ...
  const getWelcomeLines = (): { line1: string; line2: string } => {
    ...
    const defaultMessages = {
      morning: {
        line1: language === "bn" ? "হ্যালো {name}, শুভ সকাল! 👋" : "Hello {name}, Good Morning! 👋",
        line2: language === "bn" ? "আজকের কাজের জন্য {system}-এ আপনাকে স্বাগতম।" : "Welcome to {system} for today's work."
      },
      afternoon: {
        line1: language === "bn" ? "হ্যালো {name}, শুভ অপরাহ্ন! 👋" : "Hello {name}, Good Afternoon! 👋",
        line2: language === "bn" ? "{system}-এ ফিরে আসায় আপনাকে স্বাগতম।" : "Welcome back to {system}."
      },
      evening: {
        line1: language === "bn" ? "হ্যালো {name}, শুভ সন্ধ্যা! 👋" : "Hello {name}, Good Evening! 👋",
        line2: language === "bn" ? "{system}-এ ফিরে আসায় আপনাকে স্বাগতম।" : "Welcome back to {system}."
      },
      night: {
        line1: language === "bn" ? "হ্যালো {name}, শুভ রাত্রি! 👋" : "Hello {name}, Good Night! 👋",
        line2: language === "bn" ? "{system}-এ ফিরে আসায় আপনাকে স্বাগতম।" : "Welcome back to {system}."
      }
    };

    const rawLine1 = (customConfig?.line1 && customConfig.line1.trim() !== "") ? customConfig.line1 : defaultMessages[slot].line1;
    const rawLine2 = (customConfig?.line2 && customConfig.line2.trim() !== "") ? customConfig.line2 : defaultMessages[slot].line2;
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center gap-4 min-w-0">
          {systemSettings?.logoUrl ? (
            ...
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-bold text-white text-xl shadow-md shrink-0 ${
              isCustom ? "bg-gradient-to-tr from-purple-600 to-amber-500" : isDark ? "bg-emerald-600" : "bg-[#c05621]"
            }`}>
              {language === "bn" ? "বাং" : "GO"}
            </div>
          )}
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-1.5">
              <span className={`text-xs px-2.5 py-0.5 rounded-full border font-medium ${
                ...
                {isHeadOffice ? t.headOfficeBadge : `${t.subOfficeBadge}: ${currentOffice?.name}`}
              </span>
              <span className="text-slate-300 text-xs font-mono">
                {t.financialYear}: <strong className="text-white">{currentFYObj?.name}</strong>
              </span>
            </div>
            ...
            <p className={`text-xs mt-1 leading-relaxed ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-amber-200"}`}>
              {welcomeLines.line2}
            </p>

            {/* Accounting Formula Note */}
            <p className={`text-xs font-mono mt-1 opacity-80 flex items-center gap-1.5 ${isCustom ? "text-amber-300" : isDark ? "text-emerald-400" : "text-amber-300"}`}>
        ...
        <div className="flex flex-col items-start md:items-end gap-2.5 shrink-0 self-stretch md:self-auto justify-between md:justify-center">
          ...
          <div className={`flex items-center gap-2 text-xs font-medium px-3 py-1.5 rounded-xl border backdrop-blur-xs ${
            isCustom 
            ...
            <span className="font-mono text-emerald-400 font-semibold">{currentFYObj?.name || "2026-2027"}</span>
          </div>
      ...
      {balance < 0 && (
        <div className="bg-rose-50 border border-rose-200 text-rose-800 p-4 rounded-2xl flex items-center gap-3 shadow-sm">
          ...
          <div className="text-xs">
            ...
            <p className="mt-0.5">
              মোট ব্যয় বরাদ্দের চেয়ে {formatCurrency(Math.abs(balance))} বেশি হয়েছে। অনুগ্রহ করে অতিরিক্ত বরাদ্দ বা সমন্বয় প্রদান করুন।
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
        ...
        {/* Initial Allocation */}
        <div className={`p-4 rounded-2xl shadow-sm border transition-colors ${
          isCustom
          ...
          <div className="flex items-center gap-2 opacity-70 mb-2">
            <DollarSign className={`w-4 h-4 ${isCustom ? "text-amber-400" : isDark ? "text-emerald-500" : "text-emerald-600"}`} />
            <span className="text-xs font-semibold uppercase tracking-wider">{t.statAllocation}</span>
          </div>
          <div className="text-base sm:text-lg font-bold font-mono truncate">{formatCurrency(initialAllocation)}</div>
        </div>
        ...
        <div className={`p-4 rounded-2xl shadow-sm border transition-colors ${
          ...
          <div className="flex items-center gap-2 opacity-70 mb-2">
            ...
            <span className="text-xs font-semibold uppercase tracking-wider">{t.statAdditional}</span>
          </div>
          <div className="text-base sm:text-lg font-bold font-mono truncate">{formatCurrency(additionalAllocation)}</div>
        </div>
        ...
        <div className={`p-4 rounded-2xl shadow-sm border transition-colors ${
          ...
          <div className="flex items-center gap-2 opacity-70 mb-2">
            ...
            <span className="text-xs font-semibold uppercase tracking-wider">{t.statAdjustment}</span>
          </div>
          <div className="text-base sm:text-lg font-bold font-mono truncate">{formatCurrency(adjustments)}</div>
        </div>
        ...
        <div className={`p-4 rounded-2xl shadow-sm border transition-colors ${
          ...
          <div className="flex items-center gap-2 opacity-70 mb-2">
            ...
            <span className="text-xs font-semibold uppercase tracking-wider">{t.statExpense}</span>
          </div>
          <div className="text-base sm:text-lg font-bold font-mono truncate text-rose-500">{formatCurrency(totalSpent)}</div>
        </div>
        ...
        <div
          ...
          title={language === "bn" ? "পেন্ডিং ব্যয় দেখতে ক্লিক করুন" : "Click to view pending expenses"}
        >
        ...
        <div className={`p-4 rounded-2xl shadow-sm border transition-colors ${
          ...
          <div className="flex items-center gap-2 opacity-80 mb-2">
            ...
            <span className="text-xs font-semibold uppercase tracking-wider">{t.statBalance}</span>
          </div>
          <div className="text-base sm:text-lg font-bold font-mono truncate">{formatCurrency(balance)}</div>
        </div>
        ...
        <div className={`p-4 rounded-2xl shadow-sm border transition-colors ${
          ...
          <div className="flex items-center gap-2 opacity-70 mb-2">
            ...
            <span className="text-xs font-semibold uppercase tracking-wider">{t.statUtilization}</span>
          </div>
          <div className="text-base sm:text-lg font-bold font-mono truncate">{formatNumber(utilizationRate)}%</div>
        </div>
      ...
      <div className={`p-6 rounded-2xl shadow-sm border transition-colors ${
        ...
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <p className="text-xs opacity-60">
              {language === "bn" ? "খাতভিত্তিক বরাদ্দ ও খরচের সারসংক্ষেপ" : "Category allocations, expenses and balances"}
            </p>
          </div>
          <span className={`text-xs px-2.5 py-1 rounded-lg border font-mono ${
            isCustom ? "bg-[#1f183d] border-[#392b66] text-amber-300" : "bg-slate-100 text-slate-600 border-slate-200"
          }`}>
        ...
        {categorySummary.length === 0 ? (
          <div className="text-center py-8 opacity-50 text-xs">{t.noCategoriesFound}</div>
        ) : (
            ...
            {categorySummary.map(cat => (
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    <span className="text-xs font-mono opacity-50 block">{cat.code}</span>
                ...
                <div className="w-full bg-slate-700/30 rounded-full h-1.5 overflow-hidden">
                  <div 
                    className={`h-1.5 rounded-full ${
                      cat.pct > 90 ? "bg-rose-500" : isCustom ? "bg-amber-400" : "bg-[#c05621]"
                    }`}
                    style={{ width: `${Math.min(100, cat.pct)}%` }}
                  />
                </div>

                <div className={`text-xs space-y-1 font-mono pt-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-800"}`}>
                  <div className="flex justify-between">
                    <span className={isCustom ? "text-purple-300/80" : isDark ? "text-slate-400" : "text-slate-500"}>{t.allocated}:</span>
                    <span className="font-semibold">{formatCurrency(cat.allocated)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className={isCustom ? "text-purple-300/80" : isDark ? "text-slate-400" : "text-slate-500"}>{t.spent}:</span>
                    <span className="font-semibold">{formatCurrency(cat.spent)}</span>
                  </div>
                  <div className={`flex justify-between border-t pt-1 font-bold ${
                    isCustom ? "border-[#302452]" : isDark ? "border-slate-700" : "border-slate-200"
                  }`}>
                    <span className={isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-900"}>{t.remaining}:</span>
                    <span className={
                      cat.rem < 0 
      ...
      {isHeadOffice && officeSummary.length > 0 && (
        <div className={`p-6 rounded-2xl shadow-sm border transition-colors ${
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="text-xs opacity-60">{formatNumber(officeSummary.length)} {t.allOffices}</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {officeSummary.map(off => (
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    <span className="text-xs font-mono opacity-50 block">{off.code}</span>
                ...
                <div className={`text-xs space-y-1 font-mono pt-1 ${isCustom ? "text-purple-200" : isDark ? "text-slate-300" : "text-slate-800"}`}>
                  <div className="flex justify-between">
                    <span className={isCustom ? "text-purple-300/80" : isDark ? "text-slate-400" : "text-slate-500"}>{t.allocated}:</span>
                    <span className="font-semibold">{formatCurrency(off.allocated)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className={isCustom ? "text-purple-300/80" : isDark ? "text-slate-400" : "text-slate-500"}>{t.spent}:</span>
                    <span className="font-semibold">{formatCurrency(off.spent)}</span>
                  </div>
                  <div className={`flex justify-between border-t pt-1 font-bold ${
                    isCustom ? "border-[#302452]" : isDark ? "border-slate-700" : "border-slate-200"
                  }`}>
      ...
      <div className={`p-6 rounded-2xl shadow-sm border transition-colors ${
        ...
        {filteredExpenses.length === 0 ? (
          <div className="text-center py-8 opacity-50 text-xs">{t.noRecentExpenses}</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              ...
              <tbody className={`divide-y ${isCustom ? "divide-[#2b1f52]" : isDark ? "divide-slate-800" : "divide-[#e2dacd]"}`}>
                {filteredExpenses.slice(0, 5).map(e => {
                  const cat = categories.find(c => c.id === e.categoryId);
                  const off = offices.find(o => o.id === e.officeId);
                  return (
                    <tr key={e.id} className={`transition ${
                      isCustom ? "hover:bg-[#1e173d]" : isDark ? "hover:bg-slate-800/60" : "hover:bg-slate-50"
                    }`}>
                      <td className="p-3 font-mono font-semibold opacity-90">{e.voucherNo}</td>
                      <td className="p-3 opacity-75">{e.expenseDate}</td>
                      <td className="p-3 font-medium opacity-90">{cat?.name || e.categoryId}</td>
                      <td className="p-3 opacity-75">{off?.name || e.officeId}</td>
                      <td className="p-3 opacity-75 truncate max-w-xs">{e.description}</td>
